...
#define PORT_AR_RELEASE_MINOR_VERSION_IPW_C     9
#define PORT_AR_RELEASE_REVISION_VERSION_IPW_C  0
#define PORT_SW_MAJOR_VERSION_IPW_C             7
...
*/
static boolean Port_Ipw_au16GPIODirChangeability[PORT_NUM_SIUL2_INSTANCES_U8][PORT_NUM_16PIN_BLOCKS_U8][PORT_NUM_PIN_PER_BLOCK_U8];
...
void Port_Ipw_Init(const Port_ConfigType * pConfigPtr)
{
    ...
    {
        Igf_Port_Ip_Init(pConfigPtr->u16NumIgfPins, pConfigPtr->IgfIpConfigPtr);
    }
    ...
    for (u8ImcrSiulInstance = (uint8)0U; u8ImcrSiulInstance < PORT_NUM_SIUL2_INSTANCES_U8; u8ImcrSiulInstance++)
    {
        ...
        {
            for (u16NumChannel = (uint16)0U; u16NumChannel < PORT_NUM_PIN_PER_BLOCK_U8; u16NumChannel++)
            {
                Port_Ipw_au16GPIODirChangeability[u8ImcrSiulInstance][u16Counter][u16NumChannel] = FALSE;
            }
        }
    }
#endif /* (STD_ON == PORT_SET_PIN_DIRECTION_API) || ((STD_ON == PORT_SET_PIN_MODE_API) && (STD_OFF == PORT_CODE_SIZE_OPTIMIZATION)) */
                                       ...
                                       )
{
    ...
    else
    {
#endif /* (STD_ON == PORT_DEV_ERROR_DETECT) */
        /* Point to the Port Pin MSCR register address */
        u16MscrIdx           = pConfigPtr->pUsedPadConfig[PinIndex].u16MscrIdx;
        ...
        else
        {
            ...
            /* Assure the data is synchronized */
            MCAL_DATA_SYNC_BARRIER();
            /* Check the Direction changeability option */
            if ((TRUE == pConfigPtr->pUsedPadConfig[PinIndex].bDC) || \
                ((boolean)FALSE != (Port_Ipw_au16GPIODirChangeability[u8MscrSiulInstance][(uint8)(u16MscrIdx >> 4)][u8PinDescBitOffset])))
            {
#endif /* (STD_ON == PORT_DEV_ERROR_DETECT) */
                                  ...
                                  )
{
    ...
    {
        /* Check if this function was called with wrong partition */
        if ((uint32)1 != (((pConfigPtr->pau32Port_PinToPartitionMap[PinIndex]) & ((uint32)1 << PartitionId)) >> PartitionId))
        {
            (void)Det_ReportError((uint16)PORT_MODULE_ID, (uint8)PORT_INSTANCE_ID, (uint8)PORT_SETPINMODE_ID, (uint8)PORT_E_PARAM_CONFIG);
        }
                                                   ...
                                                   )
{
    ...
    /* Assign the change direction flag */
    Port_Ipw_au16GPIODirChangeability[u8ImcrSiulInstance][(uint8)(u16MscrIdx >> 4)][u8PinBitOffset] = bStatus;
...
void Port_Ipw_SetAsUsedPin(Port_PinType PinIndex, const Port_ConfigType * pConfigPtr)
{
    ...
    else
    {
        ...
        u32LocalMSCR         = pConfigPtr->pUsedPadConfig[PinIndex].u32MSCR;
        u8MscrSiulInstance   = pConfigPtr->pUsedPadConfig[PinIndex].u8MscrSiulInstance;
#if (STD_ON == PORT_VIRTWRAPPER_SUPPORT)
        uint8 u8MscrPDACSlot = pConfigPtr->pUsedPadConfig[PinIndex].u8MscrPDACSlot;
#endif
...
#endif /* (STD_ON == PORT_DEV_ERROR_DETECT) */
        u16MscrIdx           = pConfigPtr->pUsedPadConfig[PinIndex].u16MscrIdx;
        u16ImcrIdx           = pConfigPtr->pUsedPadConfig[PinIndex].u16ImcrIdx;
        u32LocalMSCR         = pConfigPtr->pUsedPadConfig[PinIndex].u32MSCR;
        u8MscrSiulInstance   = pConfigPtr->pUsedPadConfig[PinIndex].u8MscrSiulInstance;
#if (STD_ON == PORT_VIRTWRAPPER_SUPPORT)
        uint8 u8MscrPDACSlot = pConfigPtr->pUsedPadConfig[PinIndex].u8MscrPDACSlot;
