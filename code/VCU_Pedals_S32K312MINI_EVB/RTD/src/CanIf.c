...
#define CANIF_AR_RELEASE_MINOR_VERSION_C          9
#define CANIF_AR_RELEASE_REVISION_VERSION_C       0
#define CANIF_SW_MAJOR_VERSION_C                  7
#define CANIF_SW_MINOR_VERSION_C                  0
#define CANIF_SW_PATCH_VERSION_C                  1
/*==================================================================================================
*                                     FILE VERSION CHECKS
==================================================================================================*/
...
{
    const CanIf_CallbackNotifULPtrType * CanIf_UserCallbackPtr = NULL_PTR;

    if (NULL_PTR != CanIf_ConfigPtr)
    {
    #if (CANIF_DEV_ERROR_DETECT == STD_ON)
        if (CANIF_CONTROLLER_CONFIG_COUNT <= ControllerId)
        {
            (void)Det_ReportError((uint16)CANIF_MODULE_ID, CANIF_INSTANCE, CANIF_SID_CONTROLLER_BUSOFF, CANIF_E_PARAM_CONTROLLERID);
        }
        else
    #endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
        {
            CanIf_UserCallbackPtr = CanIf_PCConfig.CanIfDispatchCfgPtr;
            if (NULL_PTR != CanIf_UserCallbackPtr->UserControllerBusOff)
            {
                CanIf_UserCallbackPtr->UserControllerBusOff(ControllerId);
            }
        }
    }
}
