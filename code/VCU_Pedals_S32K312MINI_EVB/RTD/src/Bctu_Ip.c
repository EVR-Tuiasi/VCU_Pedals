...
#define BCTU_IP_AR_RELEASE_MINOR_VERSION_C       9
#define BCTU_IP_AR_RELEASE_REVISION_VERSION_C    0
#define BCTU_IP_SW_MAJOR_VERSION_C               7
#define BCTU_IP_SW_MINOR_VERSION_C               0
#define BCTU_IP_SW_PATCH_VERSION_C               1
...
#define BCTU_LIST_NUM_ELEMS(Instance)       (BCTU_IP_LISTCHR_COUNT(Instance) * 2u) /* Each LISTCHR reg contains 2 list elements */
...
static inline void Bctu_SetConvListElemWithTag(const uint32 Instance,
                                               ...
                                               const boolean LastElement);
static inline void Bctu_CheckAndCallFifoNotification(const uint32 Instance,
                                                     const uint8 FifoIdx);
static inline void Bctu_ResetRegistersDeinit(const uint32 u32Instance);
static inline void Bctu_ConfigInstance(const uint32 u32Instance,
                                       const Bctu_Ip_ConfigType * const pConfig);
#if (BCTU_IP_DYNAMIC_RESOLUTION_AVAILABLE == STD_ON)
static inline uint16 Bctu_GetResolutionScaledResult(const uint8 AdcIdx,
                                                    const uint32 RawResult);
#endif /* (BCTU_IP_DYNAMIC_RESOLUTION_AVAILABLE == STD_ON) */
...
/*LOCAL FUNCTION****************************************************************
*
* Function Name : Bctu_ConfigInstance
* Description   : Internal helper function for Bctu_Ip_Init that configures
*                 triggers, lists and FIFOs.
*
* END**************************************************************************/
static inline void Bctu_ConfigInstance(const uint32 u32Instance,
                                       const Bctu_Ip_ConfigType * const pConfig)
                  ...
                  const Bctu_Ip_ConfigType * const pConfig)
{
    ...
    DevAssert(pConfig->NumTrigConfigs <= BCTU_IP_TRGCFG_COUNT(u32Instance));
    DevAssert(pConfig->NumListItems <= BCTU_LIST_NUM_ELEMS(u32Instance));
    ...
    /* Configure triggers, lists and FIFOs */
    Bctu_ConfigInstance(u32Instance, pConfig);
...
/*LOCAL FUNCTION****************************************************************
*
* Function Name : Bctu_ResetRegistersDeinit
* Description   : Internal helper function to reset BCTU registers during
*                 deinitialization.
* END**************************************************************************/
static inline void Bctu_ResetRegistersDeinit(const uint32 u32Instance)
{
    ...
    uint8 Index;
    /* Reset LISTCHR */
    for (Index = 0u; Index < BCTU_IP_LISTCHR_COUNT(u32Instance); Index++)
    {
...
Bctu_Ip_StatusType Bctu_Ip_Deinit(const uint32 u32Instance)
{
    ...
    for (Index = 0u; Index < BCTU_IP_TRGCFG_COUNT(u32Instance); Index++)
    {
    ...
    {
        Bctu_ResetRegistersDeinit(u32Instance);
    }
                                         ...
                                         const Bctu_Ip_TrigConfigType * const pTrigConfig)
{
    ...
    DevAssert(pTrigConfig->TrigIndex < BCTU_IP_TRGCFG_COUNT(u32Instance));
...
#if (BCTU_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert(u32Instance < BCTU_INSTANCE_COUNT);
    DevAssert(u8TrigIdx < BCTU_IP_TRGCFG_COUNT(u32Instance));
#endif /* (BCTU_IP_DEV_ERROR_DETECT == STD_ON) */
                            ...
                            const uint8 u8NumItems)
{
...
#if (BCTU_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert(u32Instance < BCTU_INSTANCE_COUNT);
    DevAssert(u8StartListElemIdx < BCTU_LIST_NUM_ELEMS(u32Instance));
    ...
    {
        Bctu_SetConvListElemWithTag(u32Instance,
                                    ((Index + u8StartListElemIdx) % (uint8)BCTU_LIST_NUM_ELEMS(u32Instance)),
                                    pConvListItems[Index].AdcChanIndex,
                                    pConvListItems[Index].NextChanWaitOnTrig,
                                    (Index == (u8NumItems - 1U)) ? TRUE : FALSE);
    }
}
...
{
    uint8 Index = 0U;
    const uint8 BctuListSize = (uint8)(BCTU_LIST_NUM_ELEMS(Instance));
#if (BCTU_IP_DEV_ERROR_DETECT == STD_ON)
        DevAssert(Instance < BCTU_INSTANCE_COUNT);
        DevAssert(BctuListSize > 0u);
#endif /* (BCTU_IP_DEV_ERROR_DETECT == STD_ON) */
    if (BctuListSize > 0u)
    {
        for (Index = 0U; Index < NumCmds; Index++)
        {
            Bctu_SetConvListElemWithTag(Instance,
                                        (Index % BctuListSize),
                                        AdcCmdArray[Index].AdcChanIndex,
                                        AdcCmdArray[Index].NextChanWaitOnTrig,
                                        AdcCmdArray[Index].LastChanInList ? TRUE : FALSE );
        }
    }
}
                                 ...
                                 const uint8 u8TrigIdx)
{
    ...
    Bitmask = Bctu_GetBitmask((uint8)u8TrigIdx % BCTU_HW_REG_SIZE);
                                               ...
                                               const boolean LastElement)
{
    ...
    if ((ElemIdx % 2u) == 0u)
    {
        ...
        u32TempListChr |= (LastElement == TRUE) ? BCTU_LISTCHR_LAST_y_MASK : 0U;
    }
    else
    {
        ...
        u32TempListChr |= (LastElement == TRUE) ? BCTU_LISTCHR_LAST_y_plus_1_MASK : 0U;
    }
                                                     ...
                                                     const uint8 FifoIdx)
{
    ...
    if ((BctuBasePtr->FIFOCR & ((uint32)BCTU_FIFOCR_IEN_FIFO1_MASK << FifoIdx)) != 0u)
    {
         ...
         * correct field position.
         */
        Bctu_CheckAndCallNotification(Instance, (volatile uint32 *)&BctuBasePtr->FIFOERR, (uint32)BCTU_FIFOERR_OVR_ERR_FIFO1_MASK << (FifoIdx * 2u),
        ...
        Bctu_CheckAndCallNotification(Instance, (volatile uint32 *)&BctuBasePtr->FIFOERR, (uint32)BCTU_FIFOERR_UNDR_ERR_FIFO1_MASK << (FifoIdx * 2u),
                                     ...
                                     const uint32 FifoAddress)
{
    ...
    if (FifoConfig->FifoDmaEnable)
    {
        ...
        */
        ...
        if (TRUE == FifoConfig->FifoDmaRawEnable)
        {
            ...
            LocTransferList[6].Value  = (4U*((uint32)FifoConfig->WatermarkValue + 1U));
        }
        else
        {
...
void Bctu_Ip_IRQHandler(const uint32 Instance)
{
    ...
    {
        Bctu_CheckAndCallNotification(Instance, (volatile uint32 *)&BctuBasePtr->MSR, (uint32)BCTU_MSR_TRGF_MASK,
    ...
    for (Index = 0u; Index < FEATURE_BCTU_NUM_ADC; Index++)
    {
        ...
        {
            Bctu_CheckAndCallNotification(Instance, (volatile uint32 *)&BctuBasePtr->MSR, (uint32)BCTU_MSR_NDATA0_MASK << Index,
                (uint32)BCTU_MSR_NDATA0_CLR_MASK << Index, Bctu_Ip_axBctuState[Instance].AdcNotifications[Index].NewConvDataNotification);
            Bctu_CheckAndCallNotification(Instance, (volatile uint32 *)&BctuBasePtr->MSR, (uint32)BCTU_MSR_DATAOVR0_MASK << Index,
                (uint32)BCTU_MSR_DATAOVR0_CLR_MASK << Index, Bctu_Ip_axBctuState[Instance].AdcNotifications[Index].ConvDataOverrunNotification);
        }
        if (CheckList)
        {
            Bctu_CheckAndCallNotification(Instance, (volatile uint32 *)&BctuBasePtr->MSR, (uint32)BCTU_MSR_LIST0_Last_MASK << Index,
