...
#if (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON)
#include "Mcal.h"
#endif
...
==================================================================================================*/
/*   Can_43_FLEXCAN.c shall check if the correct version of Can_43_FLEXCAN.h is included.
...
#define CAN_43_FLEXCAN_AR_RELEASE_MINOR_VERSION_C       9
#define CAN_43_FLEXCAN_AR_RELEASE_REVISION_VERSION_C    0
#define CAN_43_FLEXCAN_SW_MAJOR_VERSION_C               7
#define CAN_43_FLEXCAN_SW_MINOR_VERSION_C               0
#define CAN_43_FLEXCAN_SW_PATCH_VERSION_C               1
...
==================================================================================================*/
    ...
    #define Can_GetUserId()       ((uint32)OsIf_GetUserId())
#else
    /* @violates @ref Can_43_FLEXCAN_c_REF_3 MISRA 2012 Advisory Rule 4.9 */
    #define Can_GetUserId()       ((uint32)0UL)
#endif
...
#endif
static boolean Can_IsControllersBusy(uint32 u32PartitionId);

static void Can_InitControllers(uint32 u32PartitionId);

#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
static boolean Can_ValidateController(uint32 u32PartitionId, uint8 u8ControllerID, uint8 u8Sid);

static boolean Can_ValidateHth(uint32 u32PartitionId, Can_HwHandleType Hth, uint8 u8Sid);

#if (CAN_43_FLEXCAN_TIMESTAMP_ENABLE == STD_ON)
static boolean Can_ValidateHrh(uint32 u32PartitionId, Can_HwHandleType Hrh, uint8 u8Sid);
...
static void Can_DeInitControllers(uint32 u32PartitionId);
...
==================================================================================================*/
    ...
    */
    static void Can_MainFunction_MultipleWrite(uint8 u8WritePoll)
    {
        ...
        u32PartitionId = Can_GetUserId();
    #if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    #if (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON)
        if (CAN_43_FLEXCAN_MAX_PARTITIONS <= u32PartitionId)
        {
            (void)Det_ReportError((uint16)CAN_43_FLEXCAN_MODULE_ID, CAN_43_FLEXCAN_INSTANCE, CAN_43_FLEXCAN_SID_MAIN_FUNCTION_WRITE, CAN_43_FLEXCAN_E_PARAM_CONFIG);
        }
        else
    #endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
    #endif /* (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON) */
        {
            if (NULL_PTR != (Can_apxConfig[u32PartitionId]))
            {
                HwObjectID = (Can_apxConfig[u32PartitionId])->Can_uHthFirstIndex;
                while (HwObjectID < CAN_43_FLEXCAN_HWOBJECT_CONFIG_COUNT)
                {
                    u8ControllerID = (Can_apxConfig[u32PartitionId])->Can_pHwObjIDToCtrlIDMap[HwObjectID];
                    Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID];
                    if ((NULL_PTR != Can_pController) && (TRUE == Can_pController->Can_bActivation) &&
                        (u8WritePoll == (Can_apxConfig[u32PartitionId])->Can_pHwObjectConfig[HwObjectID].Can_MainFuncPeriodIndex))
                    {
                        Can_43_FLEXCAN_Ipw_MainFunction_Write(Can_pController, &((Can_apxConfig[u32PartitionId])->Can_pHwObjectConfig[HwObjectID]));
                    }
                    HwObjectID++;
                }
            }
    ...
    */
    static void Can_MainFunction_MultipleRead(uint8 u8ReadPoll)
    {
        ...
        if (CAN_43_FLEXCAN_MAX_PARTITIONS <= u32PartitionId)
        {
            (void)Det_ReportError((uint16)CAN_43_FLEXCAN_MODULE_ID, CAN_43_FLEXCAN_INSTANCE, CAN_43_FLEXCAN_SID_MAIN_FUNCTION_READ, CAN_43_FLEXCAN_E_PARAM_CONFIG);
        }
        else
    #endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
    #endif /* (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON) */
        {
            if (NULL_PTR != (Can_apxConfig[u32PartitionId]))
            {
                while (HwObjectID < (Can_apxConfig[u32PartitionId])->Can_uHthFirstIndex)
                {
                    u8ControllerID = (Can_apxConfig[u32PartitionId])->Can_pHwObjIDToCtrlIDMap[HwObjectID];
                    Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID];
                    if ((NULL_PTR != Can_pController) && (TRUE == Can_pController->Can_bActivation) &&
                        (u8ReadPoll == (Can_apxConfig[u32PartitionId])->Can_pHwObjectConfig[HwObjectID].Can_MainFuncPeriodIndex))
                    {
                        Can_43_FLEXCAN_Ipw_MainFunction_Read(Can_pController, &((Can_apxConfig[u32PartitionId])->Can_pHwObjectConfig[HwObjectID]));
                    }
                    HwObjectID++;
                }
            }
...
/**
*   @brief      This sub-function performs checking if current Controller is idle or busy.
*/
static boolean Can_IsControllersBusy(uint32 u32PartitionId)
{
    ...
    {
        if (NULL_PTR != (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID])
        {
...
/**
*   @brief      This sub-function performs initializing Can Controllers.
*/
static void Can_InitControllers(uint32 u32PartitionId)
{
    boolean bCtrlBusy = FALSE;
    ...
    */
    if (TRUE == bCtrlBusy)
    {
        /* Init failed due to one or more controllers failed, reset status of all  partition's current controllers */
        for (u8ControllerID = 0U; u8ControllerID < CAN_43_FLEXCAN_CONTROLLER_CONFIG_COUNT; u8ControllerID++)
        {
            if (NULL_PTR != (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID])
            {
    ...
    {
        Can_eDriverStatus[u32PartitionId] = CAN_43_FLEXCAN_READY;
    }
#endif
}
...
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
/**
*   @brief      This sub-function performs validating Can Controller.
*/
static boolean Can_ValidateController(uint32 u32PartitionId, uint8 u8ControllerID, uint8 u8Sid)
{
    ...
    if (CAN_43_FLEXCAN_MAX_PARTITIONS <= u32PartitionId)
    {
        /* Check if ECUC partition ID is not higher than the maximum number of ECUC partitions stored in configuration */
        (void)Det_ReportError((uint16)CAN_43_FLEXCAN_MODULE_ID, CAN_43_FLEXCAN_INSTANCE, u8Sid, CAN_43_FLEXCAN_E_PARAM_CONFIG);
    }
    else
#endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
    {
        if (CAN_43_FLEXCAN_UNINIT == Can_eDriverStatus[u32PartitionId])
        {
            /*
...
/**
*   @brief      This sub-function performs validating Hth.
*/
static boolean Can_ValidateHth(uint32 u32PartitionId, Can_HwHandleType Hth, uint8 u8Sid)
{
    boolean bResult = FALSE;
...
#endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
    {
        ...
        {
            if ((Hth >= CAN_43_FLEXCAN_HWOBJECT_CONFIG_COUNT) || (Hth < (Can_apxConfig[u32PartitionId])->Can_uHthFirstIndex))
            {
                /*
                *   If development error detection for the Can module is enabled:
                ...
                */
                (void)Det_ReportError((uint16)CAN_43_FLEXCAN_MODULE_ID, CAN_43_FLEXCAN_INSTANCE, u8Sid, CAN_43_FLEXCAN_E_PARAM_HANDLE);
            }
            else
            {
            #if (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON)
                u8ControllerID = (Can_apxConfig[u32PartitionId])->Can_pHwObjIDToCtrlIDMap[Hth];
                if (NULL_PTR == (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID])
                {
                    /*
                    *   [CPR_RTD_00420.can] If DET error reporting is enabled, the <MDL> will check upon each API call if the requested resource is configured
...
#if (CAN_43_FLEXCAN_TIMESTAMP_ENABLE == STD_ON)
/**
*   @brief      This sub-function performs validating Hrh.
*/
static boolean Can_ValidateHrh(uint32 u32PartitionId, Can_HwHandleType Hrh, uint8 u8Sid)
{
    boolean bResult = FALSE;
...
#endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
    {
        ...
        {
            if (Hrh >= (Can_apxConfig[u32PartitionId])->Can_uHthFirstIndex)
            {
                /*
                *   If development error detection for the Can module is enabled:
                *   The function shall raise the error CAN_E_PARAM_HANDLE and shall
                *   return E_NOT_OK if the parameter Hrh is not a configured Hardware Receive Handle.
                */
                (void)Det_ReportError((uint16)CAN_43_FLEXCAN_MODULE_ID, CAN_43_FLEXCAN_INSTANCE, u8Sid, CAN_43_FLEXCAN_E_PARAM_HANDLE);
            }
            else
...
/**
*   @brief      This sub-function performs validating Pdu.
*/
static boolean Can_ValidatePdu(const Can_43_FLEXCAN_ControllerConfigType * Can_pController, const Can_43_FLEXCAN_HwObjectConfigType * Can_pHwObject, const Can_PduType * PduInfo)
{
...
/**
*   @brief      This sub-function performs de-initializing Can Controllers.
*/
static void Can_DeInitControllers(uint32 u32PartitionId)
{
    uint8 u8ControllerID = 0U;
...
void Can_43_FLEXCAN_Init(const Can_43_FLEXCAN_ConfigType * Config)
{
    ...
    boolean bCtrlBusy = FALSE;
#if (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON)
    boolean bValidPartitionId = FALSE;
#endif
...
#endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
    {
        if (CAN_43_FLEXCAN_READY == Can_eDriverStatus[u32PartitionId])
        {
            /*
        ...
        #if (CAN_43_FLEXCAN_PRECOMPILE_SUPPORT == STD_ON)
            #if (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON)
                if (NULL_PTR != Can_43_FLEXCAN_Config[u32PartitionId])
                {
                    /* Update config pointer with default configuration if precompile support on. */
                    Can_apxConfig[u32PartitionId] = Can_43_FLEXCAN_Config[u32PartitionId];
                    bValidPartitionId = TRUE;
                }
            #else
                /* Update config pointer with default configuration if precompile support on. */
                Can_apxConfig[u32PartitionId] = &Can_43_FLEXCAN_Config;
            #endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
                (void)Config; /* Prevent compiler warning */
        #else
            #if (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON)
                if (u32PartitionId == Config->Can_u32PartitionId)
                {
            #endif
                    /* Copy pointer to current Can Driver configuration. */
                    Can_apxConfig[u32PartitionId] = Config;
            #if (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON)
                    bValidPartitionId = TRUE;
                }
            #endif
...
void Can_43_FLEXCAN_DeInit(void)
{
    ...
    {
        /* Check if ECUC partition ID is not higher than the maximum number of ECUC partitions stored in configuration */
        (void)Det_ReportError((uint16)CAN_43_FLEXCAN_MODULE_ID, CAN_43_FLEXCAN_INSTANCE, CAN_43_FLEXCAN_SID_DEINIT, CAN_43_FLEXCAN_E_PARAM_CONFIG);
    }
    else
#endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
    {
        ...
        else
        {
            ...
            if (FALSE == bCtrlBusy)
            {
                ...
                */
                Can_eDriverStatus[u32PartitionId] = CAN_43_FLEXCAN_UNINIT;
            #endif
                Can_DeInitControllers(u32PartitionId);
                Can_apxConfig[u32PartitionId] = NULL_PTR;
            }
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
        }
    }
    ...
    */
    ...
    )
    {
        ...
        u32PartitionId = Can_GetUserId();
    #if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
        if (TRUE == Can_ValidateController(u32PartitionId, Controller, CAN_43_FLEXCAN_SID_SET_BAUDRATE))
    #endif
        {
            Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[Controller];
        #if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
...
Std_ReturnType Can_43_FLEXCAN_SetControllerMode(uint8 Controller, Can_ControllerStateType Transition)
{
    ...
    u32PartitionId = Can_GetUserId();
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    if (TRUE == Can_ValidateController(u32PartitionId, Controller, CAN_43_FLEXCAN_SID_SET_CONTROLLER_MODE))
#endif
    {
        Can_pController = (const Can_43_FLEXCAN_ControllerConfigType *)(Can_apxConfig[u32PartitionId])->Can_ppController[Controller];
        /*
...
Std_ReturnType Can_43_FLEXCAN_ListenOnlyMode(uint8 Controller, Can_43_FLEXCAN_ListenOnlyType State)
{
    ...
    u32PartitionId = Can_GetUserId();
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    if (TRUE == Can_ValidateController(u32PartitionId, Controller, CAN_43_FLEXCAN_SID_LISTEN_ONLY_MODE))
#endif
    {
        Can_pController = (const Can_43_FLEXCAN_ControllerConfigType *)(Can_apxConfig[u32PartitionId])->Can_ppController[Controller];
...
void Can_43_FLEXCAN_DisableControllerInterrupts(uint8 Controller)
{
...
#endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
    {
        ...
        else
        {
            ...
            {
            #if (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON)
                if (NULL_PTR == (Can_apxConfig[u32PartitionId])->Can_ppController[Controller])
                {
                    /*
                    *   [CPR_RTD_00420.can] If DET error reporting is enabled, the <MDL> will check upon each API call if the requested resource is configured
                    *   to be available on the current core, and in case of error will return <MDL>_E_PARAM_CONFIG.
            ...
            #endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
                {
                    ...
                    if (Can_au8DisableInterruptLevel[Controller] < (uint8)UINT8_MAX)
                    {
...
)
{
    ...
    u32PartitionId = Can_GetUserId();
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    if (TRUE == Can_ValidateController(u32PartitionId, ControllerId, CAN_43_FLEXCAN_SID_GET_CONTROLLER_ERROR_STATE))
    {
        ...
        {
#endif /* (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON) */
            Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[ControllerId];
...
)
{
...
#endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
    {
        ...
        else
        {
            ...
            {
            #if (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON)
                if (NULL_PTR == (Can_apxConfig[u32PartitionId])->Can_ppController[ControllerId])
                {
                    /*
                    *   [CPR_RTD_00420.can] If DET error reporting is enabled, the <MDL> will check upon each API call if the requested resource is configured
                    *   to be available on the current core, and in case of error will return <MDL>_E_PARAM_CONFIG.
...
)
{
    ...
    HwObjectID = Hth;
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    if (TRUE == Can_ValidateHth(u32PartitionId, Hth, CAN_43_FLEXCAN_SID_WRITE))
    {
        if (NULL_PTR == PduInfo)
        {
            /*
        ...
        else
        {
            ...
            {
                Can_pHwObject = &((Can_apxConfig[u32PartitionId])->Can_pHwObjectConfig[HwObjectID]);
            #if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    ...
    */
    ...
    void Can_43_FLEXCAN_SetClockMode(Can_43_FLEXCAN_ClockModeType eClkMode)
    {
        ...
        {
            /* Check if ECUC partition ID is not higher than the maximum number of ECUC partitions stored in configuration */
            (void)Det_ReportError((uint16)CAN_43_FLEXCAN_MODULE_ID, CAN_43_FLEXCAN_INSTANCE, CAN_43_FLEXCAN_SID_SETCLOCKMODE, CAN_43_FLEXCAN_E_PARAM_CONFIG);
        }
        else
    #endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
        {
            ...
            if (NULL_PTR != (Can_apxConfig[u32PartitionId]))
            {
                ...
                while (HwObjectID < CAN_43_FLEXCAN_HWOBJECT_CONFIG_COUNT)
                {
                    ...
                    if (NULL_PTR != Can_pController)
                    {
                        ...
                        if ((TRUE == Can_pController->Can_bActivation) && (TRUE == (Can_apxConfig[u32PartitionId])->Can_pHwObjectConfig[HwObjectID].Can_bHwObjectUsesPolling))
                        {
                            Can_43_FLEXCAN_Ipw_MainFunction_Write(Can_pController, &((Can_apxConfig[u32PartitionId])->Can_pHwObjectConfig[HwObjectID]));
                        }
                    }
...
void Can_43_FLEXCAN_MainFunction_BusOff(void)
{
    ...
    {
        /* Check if ECUC partition ID is not higher than the maximum number of ECUC partitions stored in configuration */
        (void)Det_ReportError((uint16)CAN_43_FLEXCAN_MODULE_ID, CAN_43_FLEXCAN_INSTANCE, CAN_43_FLEXCAN_SID_MAIN_FUNCTION_BUS_OFF, CAN_43_FLEXCAN_E_PARAM_CONFIG);
    }
    else
#endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
#endif /* (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON) */
...
/**
* @brief          Function called at fixed cyclic time. SID is 0x0A.
* @details        Service for performs the polling of wake-up events that are configured statically as to be polled.
*                 This routine is called by:
*                 - CanIf or an upper layer according to Autosar requirements.
*
*
* @pre            Driver must be initialized.
* @post           None
* @note           The implementation is only comply with the change according to ASR R23-11.
*                 It does not fully support wakeup functionality as the limitation of hardware capability.
*
* @api
*/
...
void Can_43_FLEXCAN_MainFunction_Wakeup(void)
{
    ...
    {
        /* Check if ECUC partition ID is not higher than the maximum number of ECUC partitions stored in configuration */
        (void)Det_ReportError((uint16)CAN_43_FLEXCAN_MODULE_ID, CAN_43_FLEXCAN_INSTANCE, CAN_43_FLEXCAN_SID_MAIN_FUNCTION_WAKEUP, CAN_43_FLEXCAN_E_PARAM_CONFIG);
    }
    else
#endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
#endif /* (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON) */
    {
        if (NULL_PTR != (Can_apxConfig[u32PartitionId]))
        {
            while (u8ControllerID < CAN_43_FLEXCAN_CONTROLLER_CONFIG_COUNT)
            {
                ...
                if (NULL_PTR != Can_pController)
                {
                    if ((TRUE == Can_pController->Can_bActivation) && (TRUE == Can_pController->Can_bWakeupUsesPolling))
                    {
                        Can_43_FLEXCAN_Ipw_MainFunction_Wakeup(Can_pController, &Can_eControllerState[u8ControllerID]);
                    }
                }
                u8ControllerID++;
            }
...
/**
* @brief          Function called at fixed cyclic time. SID is 0x0C.
* @details        Service for performs the polling of CAN status register flags to detect transition of CAN Controller state
*                 This routine is called by:
*                 - CanIf or an upper layer according to Autosar requirements.
*
*
...
void Can_43_FLEXCAN_MainFunction_Mode(void)
{
    ...
    {
        /* Check if ECUC partition ID is not higher than the maximum number of ECUC partitions stored in configuration */
        (void)Det_ReportError((uint16)CAN_43_FLEXCAN_MODULE_ID, CAN_43_FLEXCAN_INSTANCE, CAN_43_FLEXCAN_SID_MAIN_FUNCTION_MODE, CAN_43_FLEXCAN_E_PARAM_CONFIG);
    }
    else
#endif /* (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON) */
#endif /* (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON) */
...
Std_ReturnType Can_43_FLEXCAN_GetCurrentTime(uint8 ControllerId, Can_TimeStampType* timeStampPtr)
{
    ...
    u32PartitionId = Can_GetUserId();
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    if (TRUE == Can_ValidateController(u32PartitionId, ControllerId, CAN_43_FLEXCAN_SID_GET_CURRENT_TIME))
    {
...
Std_ReturnType Can_43_FLEXCAN_SetCurrentTime(uint8 ControllerId, const Can_TimeStampType* timeStampPtr)
{
    ...
    u32PartitionId = Can_GetUserId();
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    if (TRUE == Can_ValidateController(u32PartitionId, ControllerId, CAN_43_FLEXCAN_SID_SET_CURRENT_TIME))
    {
        if (NULL_PTR == timeStampPtr)
        {
...
void Can_43_FLEXCAN_EnableEgressTimeStamp(Can_HwHandleType Hth)
{
    ...
    HwObjectID = Hth;
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    if (TRUE == Can_ValidateHth(u32PartitionId, Hth, CAN_43_FLEXCAN_SID_ENABLE_EGRESS_TIMESTAMP))
#endif
    {
        u8ControllerID = (Can_apxConfig[u32PartitionId])->Can_pHwObjIDToCtrlIDMap[HwObjectID];
        Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID];
...
)
{
    ...
    HwObjectID = Hth;
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    if (TRUE == Can_ValidateHth(u32PartitionId, Hth, CAN_43_FLEXCAN_SID_GET_EGRESS_TIMESTAMP))
#endif
    {
        u8ControllerID = (Can_apxConfig[u32PartitionId])->Can_pHwObjIDToCtrlIDMap[HwObjectID];
        Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID];
        /* [SWS_Can_00534] The function requires previous controller initialization */
        if (NULL_PTR != Can_pController)
        {
        #if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
            if (NULL_PTR == timeStampPtr)
...
)
{
    ...
    HwObjectID = Hrh;
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
    if (TRUE == Can_ValidateHrh(u32PartitionId, Hrh, CAN_43_FLEXCAN_SID_GET_INGRESS_TIMESTAMP))
#endif
    {
        u8ControllerID = (Can_apxConfig[u32PartitionId])->Can_pHwObjIDToCtrlIDMap[HwObjectID];
        Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID];
...
*                   u8MbIdxOrEvent: Represent the index of Message Buffer or the event type of interrupt
*                   mbType: The message buffer type Tx, Rx, Legacy Fifo, Enhance Fifo
*/
...
)
{
...
#endif
    {
        if (NULL_PTR != Can_apxConfig[u32PartitionId])
        {
            u8ControllerID = (Can_apxConfig[u32PartitionId])->Can_pCtrlOffsetToCtrlIDMap[u8CtrlOffset];
            Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID];
            if (NULL_PTR != Can_pController)
            {
                Can_43_FLEXCAN_Ipw_ProcessHwObject(Can_pController, (Can_apxConfig[u32PartitionId]), u8MbIdxOrEvent, mbType);
            }
