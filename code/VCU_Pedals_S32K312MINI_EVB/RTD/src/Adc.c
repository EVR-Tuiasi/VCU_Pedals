...
#define ADC_AR_RELEASE_MINOR_VERSION_C          9
#define ADC_AR_RELEASE_REVISION_VERSION_C       0
#define ADC_SW_MAJOR_VERSION_C                  7
...
#define ADC_START_SEC_VAR_CLEARED_BOOLEAN_NO_CACHEABLE
#include "Adc_MemMap.h"
...
#define ADC_STOP_SEC_VAR_CLEARED_BOOLEAN_NO_CACHEABLE
#include "Adc_MemMap.h"
...
#if (ADC_VALIDATE_GLOBAL_CALL == STD_ON)
static inline Std_ReturnType Adc_ValidateGlobalCall(uint8 ServiceId,
                                                     uint8 UserId);
#endif
...
#endif
#if (ADC_VALIDATE_GLOBAL_CALL == STD_ON)
static inline void Adc_EndValidateGlobalCall(Std_ReturnType ValidCall,
                                              uint8 ServiceId,
                                              uint8 UserId);
#endif

#if (ADC_DEINIT_API == STD_ON)
static inline Std_ReturnType Adc_ValidateDeInitNotBusy(uint8 UserId);
...
#if (  (ADC_DEINIT_API == STD_ON) || (ADC_DUAL_CLOCK_MODE == STD_ON) \
    || ((ADC_PRIORITY_IMPLEMENTATION == ADC_PRIORITY_NONE) && (ADC_ENABLE_START_STOP_GROUP_API == STD_ON) && (ADC_ENABLE_QUEUING == STD_OFF)) \
    || ((ADC_HW_TRIGGER_API == STD_ON) && (ADC_ENABLE_QUEUING == STD_OFF)) \
    || (ADC_CALIBRATION == STD_ON) \
    || (ADC_SET_HW_UNIT_POWER_MODE_API == STD_ON) \
    || (ADC_POWER_STATE_SUPPORTED == STD_ON) \
    || (ADC_ENABLE_CTU_CONTROL_MODE_API == STD_ON) \
    || ((ADC_ENABLE_CTUTRIG_NONAUTO_API == STD_ON) && (ADC_ENABLE_QUEUING == STD_OFF)) \
...
static inline Std_ReturnType Adc_ValidateCheckGroupNotConversion(uint8 ServiceId,
    ...
    || (ADC_SELF_TEST == STD_ON) \
    || defined(ADC_WDG_SUPPORTED) \
    || (ADC_ENABLE_TEMPSENSE_API == STD_ON) \
    || (ADC_ENABLE_CTU_CONTROL_MODE_API == STD_ON) \
...
static inline void Adc_InitGroupsStatus(uint8 UserId);

static inline void Adc_InitUnitStatus(uint8 UserId);
...
static inline Std_ReturnType Adc_ValidateCtuFifoParam(const uint8 ServiceId,
                                                      const uint8 UserId,
                                                      const uint8 CtuLogicalId,
                                                      const Adc_CtuFifoIdxType FifoIdx,
                                                      const uint8 ResultLength);

static inline Std_ReturnType Adc_ValidateCtuControlModeApi(uint8 CtuLogicalId,
                                                           uint8 ServiceId,
                                                           uint8 UserId);
#if (ADC_IPW_BCTU_AVAILABLE == STD_ON)
static inline Std_ReturnType Adc_ValidateCtuControlModeApiByAdcUnit(const Adc_HwUnitType AdcUnit,
...
static inline Std_ReturnType Adc_CheckCurrentUserId(const Adc_ConfigType * CfgPtr,
                                                    uint8 UserId,
                                                    uint8 ServiceId);
...
static inline Std_ReturnType Adc_CheckSetChannelCtuTriggers(const Adc_GroupType GroupIndex,
                                                            ...
                                                            const Adc_GroupDefType *const Channel,
...
#if ((ADC_ENABLE_CTU_CONTROL_MODE_API == STD_ON) && (ADC_DEV_ERROR_DETECT == STD_ON) && (ADC_IPW_BCTU_AVAILABLE == STD_ON))
static inline boolean Adc_ValidateTriggerLoopMode(const uint8 UserId,
                                                  const uint32 CtuPhysicalId,
                                                  const uint8 TriggerId);
#endif /* ((ADC_ENABLE_CTU_CONTROL_MODE_API == STD_ON) && (ADC_DEV_ERROR_DETECT == STD_ON) && (ADC_IPW_BCTU_AVAILABLE == STD_ON)) */
                                                     ...
                                                     uint8 UserId)
{
    ...
    if (ADC_STATE_UNINIT == Adc_aeGlobalState[UserId])
    {
                                          ...
                                          uint8 UserId)
{
    ...
    Adc_GroupType GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
...
#endif /* ADC_ENABLE_CH_DISABLE_CH_NONAUTO_API == STD_ON */
        {
            if (ADC_TRIGG_SRC_SW != Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].TriggerSource)
            {
                                            ...
                                            uint8 UserId)
{
    ...
    /* Get the mapping index of group in the current partition */
    GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
    LogicalHwUnitId = Adc_apxCfgPtr[UserId]->GroupsPtr[(GroupIndex)].AdcLogicalUnitId;

    switch (ServiceId)
    {
        case ADC_STARTGROUPCONVERSION_ID:
        {
            if (ADC_CONV_TYPE_NORMAL == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Type)
            {
                                        ...
                                        uint8 UserId)
{
    ...
    const Adc_HwUnitType LogicalHwUnitId  = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AdcLogicalUnitId;
    const Adc_HwUnitType PhysicalHwUnitId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcPhysicalIdArr[LogicalHwUnitId];
    ...
    switch (ServiceId)
    {
...
#endif /* (ADC_IPW_SDADC_IS_USED == STD_ON) */
            {
                if (ADC_CONV_MODE_CONTINUOUS == (Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Mode))
                {
        ...
        case ADC_DISABLE_CHANNEL_ID:
        {
            if (ADC_CONV_TYPE_INJECTED == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Type)
            {
                Adc_ReportValidationError(ReturnValue, ServiceId, (uint8)ADC_E_WRONG_CONV_MODE);
            }
            break;
    ...
    switch (ServiceId)
    {
        ...
        case ADC_DISABLE_CHANNEL_ID:
        {
            if ((Adc_GroupType)ADC_ENABLE_CH_DISABLE_CH_INVALID_GROUP_INDEX == \
               Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].EnableChDisableChGroupIndex
              )
            {
                                                            ...
                                                            uint8 UserId)
{
    ...
    switch (ServiceId)
    {
        ...
        case ADC_DISABLE_CHANNEL_ID:
        {
            GroupChIndexPtr = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AssignmentPtr;
            /* Check if ChannelId is valid: assigned to that group */
            for (Index = 0U; Index < Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AssignedChannelCount; Index++)
            {
                                                     ...
                                                     uint8 UserId)
{
    ...
    {
       Adc_ValidateTriggerSrc(&ValidationStatus, ServiceId, Group, UserId);
    }
#if (ADC_ENABLE_START_STOP_GROUP_API == STD_ON)
    /* Validate Queue */
    if ((0U != (ErrorIdList&ADC_E_QUEUE_FULL_LIST)) && (FALSE == ValidationStatus.EndValidations))
    {
        Adc_ValidateQueueNotFull(&ValidationStatus, ServiceId, Group, UserId);
    }
#endif
#if ((ADC_ENABLE_CH_DISABLE_CH_NONAUTO_API == STD_ON) || (ADC_HW_TRIGGER_API == STD_ON) \
    || (ADC_ENABLE_CTUTRIG_NONAUTO_API == STD_ON) \
    ...
    {
        Adc_ValidateConvMode(&ValidationStatus, ServiceId, Group, UserId);
    }
#endif
#if (ADC_ENABLE_CH_DISABLE_CH_NONAUTO_API == STD_ON)
    if ((0U != (ErrorIdList&ADC_E_WRONG_ENABLE_CH_DISABLE_CH_GROUP_LIST)) && (FALSE == ValidationStatus.EndValidations))
    {
        Adc_ValidateEnableDisbleGroupList(&ValidationStatus, ServiceId, Group, UserId);
    }
#endif /* ADC_ENABLE_CH_DISABLE_CH_NONAUTO_API == STD_ON */
                                                            ...
                                                            uint8 UserId)
{
    ...
    Std_ReturnType ValidUserId;

    if (NULL_PTR == Adc_apxCfgPtr[UserId])
    {
#if (ADC_DEV_ERROR_DETECT == STD_ON)
        /* Report error to DET */
    ...
    {
        ValidUserId = Adc_CheckCurrentUserId(Adc_apxCfgPtr[UserId], UserId, (uint8)ADC_SET_HW_UNIT_POWER_MODE_ID);
        if ((Std_ReturnType)E_OK == ValidUserId)
        {
                                              ...
                                              uint8 UserId)
{
    if ((Std_ReturnType)E_OK == ValidCall)
    {
        ...
        {
            Adc_aeGlobalState[UserId] = ADC_STATE_UNINIT;
        }
        else
        {
            Adc_aeGlobalState[UserId] = ADC_STATE_IDLE;
        }
    }
}
#endif /* ADC_VALIDATE_GLOBAL_CALL == STD_ON */
                                                         ...
                                                         uint8 UserId)
{
    ...
    Std_ReturnType ValidState = (Std_ReturnType)E_NOT_OK;
    Adc_GroupType GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
    Adc_HwUnitType LogicalHwUnitId = Adc_apxCfgPtr[UserId]->GroupsPtr[(GroupIndex)].AdcLogicalUnitId;
    ...
    /* SWS_Adc_00427 */
    else if (((Conversion != ADC_IDLE) \
            && (Conversion != ADC_STREAM_COMPLETED) \
             ) && (TRUE == Implicitly) \
            )
    {
        Adc_ReportDetRuntimeError((uint8)ADC_STARTGROUPCONVERSION_ID, (uint8)ADC_E_BUSY);
                                                             ...
                                                             uint8 UserId)
{
    ...
    Adc_GroupConvType eGroupType = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Type;
#if (ADC_SOFTWARE_INJECTED_CONVERSIONS_USED == STD_ON)
    ...
    switch (ServiceId)
    {
        ...
        case ADC_ENABLECTUTRIGGER_ID:
        {
            ...
            if ((Adc_QueueIndexType)0U == NoGroupsInSwInjectedQueue)
            {
                ...
                else if ( (Group == OngoingHwGroupId)
                          &&
                          (
                            (ADC_HWTRIGGER_DISABLED == Adc_axGroupStatus[OngoingHwGroupId].HwTriggering)
                            ||
                            (ADC_CONV_TYPE_NORMAL == eGroupType)
                            ||
                            (Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].HwTriggerSource <= (Adc_HwTriggerTimerType)ADC_IPW_MAX_CTU_TRIG_SOURCE_ID)
                          )
                        )
                {
                    /* HW group is already enabled, but it's compatible with CTU trigger.
                       Except for hardware injected groups (which is using EXT_TRIG), they can be aborted by CTU triggers.
                     */
                    ValidState = (Std_ReturnType)E_OK;
                }
                else
                                                                 ...
                                                                 uint8 UserId)
{
    ...
    for (GroupIter = 0U; GroupIter < (Adc_GroupType)Adc_apxCfgPtr[UserId]->GroupCount; GroupIter++)
    {
        GroupId = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIter].GroupId;
        Conversion = Adc_axGroupStatus[GroupId].Conversion;
        /*  Check if ADC is still converting */
        /* SWS_Adc_00112 */
        if ((ADC_IDLE != Conversion) \
            && (ADC_STREAM_COMPLETED != Conversion) \
           )
        {
            Adc_ReportDetRuntimeError(ServiceId, (uint8)ADC_E_BUSY);
...
static inline Std_ReturnType Adc_ValidateDeInitNotBusy(uint8 UserId)
{
    ...
    {
        if (Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcHwUnitArr[LogicalHwUnitId] == (uint8)STD_ON) /* HW unit enabled on current partition */
        {
    ...
    {
        if ((Std_ReturnType)E_OK == Adc_ValidateCheckGroupNotConversion(ADC_DEINIT_ID, UserId))
        {
            ValidState = (Std_ReturnType)E_OK;
        }
    }
                                                                    ...
                                                                    uint8 UserId)
{
    ...
    /* Get the mapping index of group in the current partition */
    GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
    if ((ADC_CONV_MODE_ONESHOT == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Mode) \
        || ((ADC_CONV_MODE_CONTINUOUS == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Mode) && \
        (ADC_ACCESS_MODE_STREAMING == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AccessMode) && \
        (ADC_STREAM_BUFFER_LINEAR == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].BufferMode)) \
       )
    {
    ...
    (ADC_SOFTWARE_INJECTED_CONVERSIONS_USED == STD_ON) \
    || (ADC_ENABLE_CTU_CONTROL_MODE_API == STD_ON) \
    )
    ...
    if (ADC_INVALID_HW_GROUP_ID != OngoingHwGroupId)
    {
        TypeSwGroup = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Type;
        HwGroupId = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[OngoingHwGroupId];
        TypeHwGroup = Adc_apxCfgPtr[UserId]->GroupsPtr[HwGroupId].Type;
    ...
    else
    {
        ...
        ValidState = Adc_ValidateNotBusyNoPrio(Group, Implicitly, UserId);
#else /* ADC_PRIORITY_IMPLEMENTATION != ADC_PRIORITY_NONE */
        ValidState = Adc_ValidateNotBusyWithPrio(Group, Implicitly, UserId);
#endif  /* ADC_PRIORITY_IMPLEMENTATION != ADC_PRIORITY_NONE */
    }
    return ValidState;
}
...
#endif /* (ADC_ENABLE_CTU_CONTROL_MODE_API == STD_ON) */
    {
        ValidState = Adc_ValidateNotBusyEnableHwTrig(ADC_ENABLEHARDWARETRIGGER_ID, Group, UserId);
    }
...
* @details      This function checks for Uninit, valid UserId, and that Group is in range.
*
* @param[in]    ServiceId     The service id of the caller function
* @param[in]    Group         The group id
* @param[in]    UserId       Current UserId
                                                      ...
                                                      uint8 UserId)
{
    ...
    Std_ReturnType ValidUserId = (Std_ReturnType)E_NOT_OK;
    ...
    {
        ValidUserId = Adc_CheckCurrentUserId(Adc_apxCfgPtr[UserId], UserId, ServiceId);
        if ((Std_ReturnType)E_OK == ValidUserId)
        {
            ...
            else
            {
                /* Get the mapping index of group in the current partition */
                GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
                if (Group != Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].GroupId)
                {
                                                     ...
                                                     uint8 UserId)
{
    ...
    else
    {
        ...
        if ((Std_ReturnType)E_OK == ValidUserId)
        {
            if (Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcHwUnitArr[Unit] == (uint8)STD_ON) /* HW unit enabled on current partition */
            {
                                                 ...
                                                 uint8 UserId)
{
    ...
    switch (ServiceId)
    {
        ...
        {
            if (NULL_PTR == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Notification)
            {
...
static inline void Adc_InitGroupsStatus(uint8 UserId)
{
    ...
    for (GroupCnt = 0U; GroupCnt < Adc_apxCfgPtr[UserId]->GroupCount; GroupCnt++)
    {
        GroupId = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupCnt].GroupId;
...
#if (ADC_SETCHANNEL_API == STD_ON)
        Adc_axRuntimeGroupChannel[GroupId].ChannelPtr = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupCnt].AssignmentPtr;
        Adc_axRuntimeGroupChannel[GroupId].ChannelCount = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupCnt].AssignedChannelCount;
...
#if (ADC_POWER_STATE_SUPPORTED == STD_ON)
    Adc_aeCurrentState[UserId] = ADC_FULL_POWER;
    Adc_aeTargetState[UserId] = ADC_NODEFINE_POWER;
#endif /* (ADC_POWER_STATE_SUPPORTED == STD_ON) */

#if (ADC_DUAL_CLOCK_MODE == STD_ON)
#if (ADC_SET_ADC_CONV_TIME_ONCE == STD_OFF)
    Adc_aeClockMode[UserId] = ADC_NORMAL;
#endif /* (ADC_SET_ADC_CONV_TIME_ONCE == STD_OFF) */
#endif /* (ADC_DUAL_CLOCK_MODE == STD_ON) */
}

/**
* @brief        This function check UserId is used.
* @details      This function check UserId is used.
*
* @param[in]      CfgPtr           Configuration data pointer.
* @param[in]      UserId           Current UserId
*
* @return       Std_ReturnType  Standard return type.
* @retval       E_OK:           UserId is valid
* @retval       E_NOT_OK:       UserId is not valid
*/
static inline Std_ReturnType Adc_CheckCurrentUserId(const Adc_ConfigType * CfgPtr,
                                                    uint8 UserId,
                                                    uint8 ServiceId)
...
#endif /* ADC_DEV_ERROR_DETECT == STD_ON */
        (void)ServiceId;
        if (UserId == CfgPtr->UserId)
        {
            for (Index = 0U; Index < CfgPtr->AssignedPartitionCount; Index++)
            {
                if (UserId == CfgPtr->AssignmentPartitionPtr[Index])
                {
                    ValidUserId = (Std_ReturnType)E_OK;
                    break;
                }
            }
        }
                                                 ...
                                                 uint8 UserId)
{
    ...
    Adc_GroupPriorityType GroupPriority = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Priority;
    ...
    while (Adc_apxCfgPtr[UserId]->GroupsPtr[(Adc_axUnitStatus[Unit].SwNormalQueue[Pos])].Priority >= GroupPriority)
    {
        Pos++;
        if (Pos >= QueueIndex)
        {
    ...
    if (0U == Pos)
    {
        ...
        TimeOutStatus = Adc_Ipw_StopCurrentConversion(Unit, Adc_axUnitStatus[Unit].SwNormalQueue[0U], UserId);
    }
#if (ADC_QUEUE_MAX_DEPTH_MAX != 1U)
...
/* First available slot in the Queue */
    ...
    if (ADC_CONV_TYPE_INJECTED == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Type)
    {
        ...
        if ((ADC_DMA == Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcDmaInterruptSoftware[Unit]) || \
            ((uint8)STD_ON == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AdcExtDMAChanEnable) \
           )
        {
        ...
        Adc_Ipw_StartInjectedConversion(Unit, UserId);
...
#endif /* ADC_SOFTWARE_INJECTED_CONVERSIONS_USED == STD_ON */
    {
        ...
        {
            /* Queue is NOT Empty */
            TimeOutStatus = Adc_InsertIntoQueue(Unit, Group, UserId);
    ...
    /* In this case all the StartConversion should be rejected by DET */
        ...
        if ((Adc_axUnitStatus[Unit].SwNormalQueue[0U] == Group) || (0U == SwNormalQueueIndex))
        {
            ...
            Adc_Ipw_StartNormalConversion(Unit, UserId);
#if (ADC_ENABLE_QUEUING == STD_ON)
        }
        else
        {
...
)
{
    ...
    if (ADC_CONV_TYPE_INJECTED == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Type)
    {
        ...
        if (Group == Adc_axUnitStatus[Unit].SwInjectedQueue[0U])
        {
            ...
            TimeOutStatus = Adc_Ipw_StopCurrentConversion(Unit, Group, UserId);
                                                  ...
                                                  uint8 UserId)
{
    ...
    Mode = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].Mode;
    AccessMode = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AccessMode;
    BufferMode = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].BufferMode;
    Conversion = Adc_axGroupStatus[Group].Conversion;
      ...
      )
    {
        ...
        if (Adc_axUnitStatus[Unit].SwNormalQueueIndex > (Adc_QueueIndexType)0U)
        {
            Adc_axUnitStatus[Unit].SwNormalQueueIndex = (Adc_QueueIndexType)0U;
        }
#endif /* (ADC_ENABLE_QUEUING == STD_ON) */
    }
    ...
    {
#if (ADC_ENABLE_QUEUING == STD_ON)
        TimeOutStatus = Adc_StopSwGroupConversion(&RemovedPos, Group, Unit, UserId);
#else
        TimeOutStatus = Adc_StopSwGroupConversion(Group, Unit, UserId);
#endif /* ADC_ENABLE_QUEUING == STD_ON */
...
{
    Adc_GroupType GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
    if ((uint8)STD_ON == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AdcWithoutInterrupt)
    {
        Adc_UpdateStatusReadGroupNoInt(Group, Flag, UserId);
    }
    else
    {
        Adc_UpdateStatusReadGroupInt(Group, UserId);
    }
}
    ...
    if (TRUE == Flag)
    {
        ...
        else
        {
            ...
            if (ADC_TRIGG_SRC_SW == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].TriggerSource)
            {
                ...
                Adc_UpdateSwQueueIndexNoInt(Group, UserId);
            }
        }
    }
    #if (ADC_ENABLE_LIMIT_CHECK == STD_ON)
...
#endif /* ADC_SOFTWARE_INJECTED_CONVERSIONS_USED == STD_ON */
    {
        if (Adc_axUnitStatus[LogicalHwUnitId].SwNormalQueueIndex > (Adc_QueueIndexType)0U)
        {
            ...
            if (0U == SwNormalQueueIndex)
            {
                ...
                {
                    Adc_Ipw_StartNormalConversion(LogicalHwUnitId, UserId);
                }
            }
#endif /* (ADC_ENABLE_START_STOP_GROUP_API == STD_ON) */
#else
                                                ...
                                                uint8 UserId)
{
    ...
    else if (ADC_STREAM_COMPLETED == Adc_axGroupStatus[Group].Conversion)
    {
        ...
        {
            if ((ADC_ACCESS_MODE_STREAMING == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AccessMode)&& \
               (ADC_STREAM_BUFFER_LINEAR == Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].BufferMode)
              )
            {
                /* SWS_Adc_00330 - SWS_Adc_00221 */
...
{
#if (ADC_ENABLE_CTUTRIG_NONAUTO_API == STD_ON)
    /* Active CTU trigger state to sync with subsequent calls of "Adc_EnableCTUTrigger" */
    const Adc_GroupType          GroupIndex    = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
    const Adc_HwTriggerTimerType TriggerSource = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].HwTriggerSource;
    uint8 TrigIdx = 0U;

    /* Check if the HW trigger source comes from CTU or BCTU */
    if (TriggerSource <= (Adc_HwTriggerTimerType)ADC_IPW_MAX_CTU_TRIG_SOURCE_ID)
    {
        /* Map TriggerSource to TriggerIndex.
           (checking for returned result is not needed, because the "TriggerSource" comes from generated configuration)
         */
        (void)Adc_Ipw_MapCTUTriggerIndex(TriggerSource, UserId, &TrigIdx);
        /* Mark trigger as enabled */
        Adc_au8CtuGroupTriggersActive[Unit][TrigIdx] = 1U;
    }
#else
    (void)UserId;
#endif /* (ADC_ENABLE_CTUTRIG_NONAUTO_API == STD_ON) */
...
{
#if (ADC_ENABLE_CTUTRIG_NONAUTO_API == STD_ON)
    /* De-active CTU trigger state to sync with subsequent calls of "Adc_EnableCTUTrigger" */
    const Adc_GroupType          GroupIndex    = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
    const Adc_HwTriggerTimerType TriggerSource = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].HwTriggerSource;
    uint8 TrigIdx = 0U;
    ...
    if (TriggerSource <= (Adc_HwTriggerTimerType)ADC_IPW_MAX_CTU_TRIG_SOURCE_ID)
    {
        ...
        Adc_au8CtuGroupTriggersActive[Unit][TrigIdx] = 0U;
    }
#else
    (void)UserId;
#endif /* (ADC_ENABLE_CTUTRIG_NONAUTO_API == STD_ON) */
                                                    ...
                                                    uint8 UserId)
{
    ...
    if (0U == Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcUnitSupportCtuControlMode[Unit])
    {
        /* CPR_RTD_00272.adc */
        Adc_ReportDetError(ServiceId, (uint8)ADC_E_PARAM_UNIT);
                                                           ...
                                                           uint8 UserId)
{
    ...
    Adc_HwUnitType AdcLogicalId;
    Adc_HwUnitType AdcPhysicalId;
    boolean CtuControlModeEnabled = FALSE;
    uint8 CtuPhysicalIdEnabled;
    uint8 CtuPhysicalIdExpected;

    if (NULL_PTR == Adc_apxCfgPtr[UserId])
    {
        /* Adc_Init() was not called before */
        Adc_ReportDetError(ServiceId, (uint8)ADC_E_UNINIT);
        Status = E_NOT_OK;
    }
    else if(CtuLogicalId >= ADC_IPW_CTU_MAX_HW_UNITS_CFG)
    {
        /* CtuLogicalId out of range */
        Adc_ReportDetError(ServiceId, (uint8)ADC_E_INVALID_PARAM);
        Status = E_NOT_OK;
    }
    else
    {
        CtuPhysicalIdExpected = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.CtuLogToPhyId[CtuLogicalId];
        for (AdcLogicalId = 0u; AdcLogicalId < ADC_IPW_MAX_HW_UNITS_CFG; AdcLogicalId++)
        {
            if (TRUE == Adc_axUnitStatus[AdcLogicalId].CtuControlOngoing)
            {
                /* Get CTU Instance corresponding to ADC instance */
                AdcPhysicalId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcPhysicalIdArr[AdcLogicalId];
                CtuPhysicalIdEnabled = Adc_Ipw_AdcPhysicalToCtuPhysical(AdcPhysicalId);
                /* Check if this is the expected Ctu instance */
                if (CtuPhysicalIdEnabled == CtuPhysicalIdExpected)
                {
                    CtuControlModeEnabled = TRUE; /* CtuControl Mode is considered enabled, if it is enabled for at least one AdcHwUnit which associates with that CTU */
                    break;
                }
            }
        }
        /* Raise error if the expected CTU instance is not enabled yet */
                                                                    ...
                                                                    const uint8 UserId)
{
    ...
    {
        /* Get the expected Ctu instance */
        CtuPhysicalIdExpected = Adc_Ipw_AdcPhysicalToCtuPhysical(Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcPhysicalIdArr[AdcUnit]);
        ...
        for (AdcLogicalId = 0u; AdcLogicalId < ADC_IPW_MAX_HW_UNITS_CFG; AdcLogicalId++)
        {
            ...
            {
                /* Get CTU Instance corresponding to ADC instance */
                CtuPhysicalIdEnabled = Adc_Ipw_AdcPhysicalToCtuPhysical(Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcPhysicalIdArr[AdcLogicalId]);
                /* Check if this is the expected Ctu instance */
                if (CtuPhysicalIdEnabled == CtuPhysicalIdExpected)
                {
                    CtuControlModeEnabled = TRUE; /* CtuControl Mode is considered enabled, if it is enabled for at least one AdcHwUnit which associates with that CTU */
...
{
    Std_ReturnType Status = E_OK;
    const uint8_t CtuFifoDepth[ADC_IPW_NUM_CTU_INSTANCES][ADC_IPW_NUM_CTU_FIFO] = ADC_IPW_CTU_FIFO_DEPTH;
    uint8_t CtuHwUnitId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.CtuLogToPhyId[CtuLogicalId];

    if (FifoIdx >= ADC_IPW_NUM_CTU_FIFO)
    {
        Adc_ReportDetError(ServiceId, ADC_E_PARAM_CONFIG);
        Status = E_NOT_OK;
    }
    else if (CtuFifoDepth[CtuHwUnitId][FifoIdx] < ResultLength)
    {
        Adc_ReportDetError(ServiceId, ADC_E_PARAM_CONFIG);
        Status = E_NOT_OK;
    }
#ifdef ADC_CTU_DMA_SUPPORTED
    else if (TRUE == Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->CtuConfigControlMode[CtuLogicalId]->FifoConfigs[FifoIdx].FifoDmaEnable)
    {
        Adc_ReportDetError(ServiceId, ADC_E_DMA_FIFO_TRANSFER);
        Status = E_NOT_OK;
    }
#endif /* ADC_CTU_DMA_SUPPORTED */
    else
    {
        ; /* Empty else branch to avoid MISRA */
    }

#ifndef ADC_CTU_DMA_SUPPORTED
    (void)UserId;
    (void)CtuLogicalId;
#endif /* ADC_CTU_DMA_SUPPORTED */
                                                     ...
                                                     uint8 UserId)
{
    ...
    else if (NULL_PTR == Adc_apxCfgPtr[UserId])
    {
    ...
    {
        ValidUserId = Adc_CheckCurrentUserId(Adc_apxCfgPtr[UserId], UserId, ServiceId);
        if ((Std_ReturnType)E_NOT_OK == ValidUserId)
        {
#if (ADC_DEV_ERROR_DETECT == STD_ON)
            /* CPR_RTD_00420.adc */
            Adc_ReportDetError(ServiceId, (uint8)ADC_E_PARAM_CONFIG);
                                                           ...
                                                           uint8 UserId)
{
    ...
    if (Adc_aeTargetState[UserId] >= ADC_NODEFINE_POWER)
    {
                                                            ...
                                                            const uint8 UserId)
{
    ...
    if (Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].TriggerSource == ADC_TRIGG_SRC_HW)
    {
        /* Check number of channel in the list if the HW trigger source comes from CTU or BCTU */
        if (Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].HwTriggerSource <= (Adc_HwTriggerTimerType)ADC_IPW_MAX_CTU_TRIG_SOURCE_ID)
        {
#if (ADC_CTU_HW_TRIGGER_OPTIMIZATION == STD_OFF)
            /* Number channel should be less than maximum element of the CTU or BCTU List */
            if (NumberOfChannel > Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->CtuListMaxNumElems[LogicalHwUnitId])
            {
                Adc_ReportDetError(ADC_SETCHANNEL_ID, (uint8)ADC_E_PARAM_CHANNEL);
                StatusChecks = (Std_ReturnType)E_NOT_OK;
            }
            ...
            {
                /* Physical channel index should be less than or equal to the maximum value of the ADC Port channel number in CTU or BCTU command */
                PhysicalChannelId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcConfigPtrArr[LogicalHwUnitId]->ChannelConfigsPtr[Channel[Index]].ChanIndex;
                if (ADC_IPW_CTU_MAX_ADC_CHANNEL_ID < PhysicalChannelId)
                {
                    Adc_ReportDetError(ADC_SETCHANNEL_ID, ADC_E_PARAM_CONFIG);
                    break;
                }
            }
#else
                                                        ...
                                                        const uint8 UserId)
{
    ...
    {
        ValidUserId = Adc_CheckCurrentUserId(Adc_apxCfgPtr[UserId], UserId, (uint8)ADC_SETCLOCKMODE_ID);
        if ((Std_ReturnType)E_NOT_OK == ValidUserId)
        {
            /* CPR_RTD_00420.adc */
...
static inline boolean Adc_ValidateTriggerLoopMode(const uint8 UserId,
                                                  ...
                                                  const uint8 TriggerId)
{
    boolean LoopModeEnabled = FALSE;
    const Adc_ConfigType * const CfgPtr = Adc_apxCfgPtr[UserId];
    const uint32 NumTrigConfigs = (uint32)(CfgPtr->AdcIpwConfigPtr->CtuConfigControlMode[CtuPhysicalId]->NumTrigConfigs);
    uint32 Num;

    for (Num = 0; Num < NumTrigConfigs; Num++)
    {
        /* Check whether TriggerId exists or not  */
        if (CfgPtr->AdcIpwConfigPtr->CtuConfigControlMode[CtuPhysicalId]->TrigConfigs[Num].TrigIndex == TriggerId)
        {
            /* Check if LoopEn of TriggerId is enable or disable */
            LoopModeEnabled = CfgPtr->AdcIpwConfigPtr->CtuConfigControlMode[CtuPhysicalId]->TrigConfigs[Num].LoopEn;
            break;
        }
    }
...
void Adc_Init(const Adc_ConfigType * ConfigPtr)
{
    ...
    UserId = Adc_GetUserID();
    UserIdTemp = UserId; /* Used to avoid MISRA */
#if (ADC_DEV_ERROR_DETECT == STD_ON) && (ADC_VALIDATE_GLOBAL_CALL == STD_ON)
        ValidStatus = Adc_ValidateGlobalCall(ADC_INIT_ID, UserId);
...
#if (ADC_PRECOMPILE_SUPPORT == STD_ON)
                ValidUserId = Adc_CheckCurrentUserId(Adc_ConfigVariantPredefined[UserId], UserIdTemp, (uint8)ADC_INIT_ID);
#else
                ValidUserId = Adc_CheckCurrentUserId(ConfigPtr, UserId, (uint8)ADC_INIT_ID);
#endif /* (ADC_PRECOMPILE_SUPPORT == STD_ON) */
                if ((Std_ReturnType)E_OK == ValidUserId)
                {
#if (ADC_PRECOMPILE_SUPPORT == STD_ON)
                    (void)ConfigPtr;
                    /* ADC342 */
                    const Adc_ConfigType * predefined = Adc_ConfigVariantPredefined[UserId];
                    Adc_apxCfgPtr[UserId] = predefined;
#else
                    /* SWS_Adc_00054 */
                    Adc_apxCfgPtr[UserId] = ConfigPtr;
#endif /* (ADC_PRECOMPILE_SUPPORT == STD_ON) */
                    /* Initialize the unit status for all units */
                    Adc_InitUnitStatus(UserId);
                    /* Initialize the group status for all groups */
                    Adc_InitGroupsStatus(UserId);
                    /* Call the low level function to initialize driver */
                    TimeOutStatus = Adc_Ipw_Init(Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr, UserIdTemp);
        ...
        if ((Std_ReturnType)E_OK == ValidStatus)
        {
            Adc_EndValidateGlobalCall(ValidStatus, ADC_INIT_ID, UserId);
        }
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) && (ADC_VALIDATE_GLOBAL_CALL == STD_ON) */
}

/* SWS_Adc_00419 SWS_Adc_91000 */
...
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_SETUPRESULTBUFFER_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */
        ...
        GroupPtr = &Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex];
#endif /* ADC_DMA_SUPPORTED */
...
void Adc_DeInit(void)
{
    ...
    UserIdTemp = UserId; /* Used to avoid MISRA */
#if (ADC_VALIDATE_GLOBAL_CALL == STD_ON)
    ValidStatus = Adc_ValidateGlobalCall(ADC_DEINIT_ID, UserId);
    if ((Std_ReturnType)E_OK == ValidStatus)
    {
#endif /* ADC_VALIDATE_GLOBAL_CALL == STD_ON */
        ValidUserId = Adc_CheckCurrentUserId(Adc_apxCfgPtr[UserId], UserIdTemp, (uint8)ADC_DEINIT_ID);
        if ((Std_ReturnType)E_OK == ValidUserId)
        {
            ValidStatus = Adc_ValidateDeInitNotBusy(UserId);
            if ((Std_ReturnType)E_OK == ValidStatus)
            {
                TempReturn = Adc_Ipw_DeInit(UserId);
    ...
    Adc_EndValidateGlobalCall(ValidStatus, ADC_DEINIT_ID, UserId);
#endif /* ADC_VALIDATE_GLOBAL_CALL == STD_ON */
}
#endif /* (ADC_DEINIT_API == STD_ON) */
...
void Adc_StartGroupConversion(Adc_GroupType Group)
{
    ...
    UserId = Adc_GetUserID();
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_STARTGROUPCONVERSION_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */
        /* Get the mapping index of group in the current partition */
        GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];

        if ((Std_ReturnType)E_OK == Adc_ValidateStateStartGroupConvNotBusy(Group, UserId))
        {
#if (ADC_VALIDATE_PARAMS == STD_ON)
               ...
               )
            {
                ...
                LogicalHwUnitId = Adc_apxCfgPtr[UserId]->GroupsPtr[(GroupIndex)].AdcLogicalUnitId;
                /* Update queue */
                Adc_UpdateStatusStartConversion(Group, LogicalHwUnitId, UserId);
...
void Adc_StopGroupConversion(Adc_GroupType Group)
{
    ...
    UserId = Adc_GetUserID();
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_STOPGROUPCONVERSION_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */

        if ((Std_ReturnType)E_OK == Adc_ValidateStateNotIdle(ADC_STOPGROUPCONVERSION_ID, Group, UserId))
        {
#if (ADC_VALIDATE_PARAMS == STD_ON)
               ...
               )
            {
                ...
                /* Get the unit to which the group belongs to */
                LogicalHwUnitId = Adc_apxCfgPtr[UserId]->GroupsPtr[(GroupIndex)].AdcLogicalUnitId;
                Adc_UpdateStatusStopConversion(Group, LogicalHwUnitId, UserId);
                             ...
                             Adc_ValueGroupType * DataBufferPtr)
{
    ...
    UserId = Adc_GetUserID();
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_VALUEREADGROUP_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */
        ...
        if ((Std_ReturnType)E_OK == Adc_ValidatePtr(ADC_VALUEREADGROUP_ID, DataBufferPtr))
        {
            ...
            if ((Std_ReturnType)E_OK == Adc_ValidateStateNotIdle(ADC_VALUEREADGROUP_ID, Group, UserId))
            {
                /* SWS_Adc_00447 */
                GroupRet = Adc_Ipw_ReadGroup(Group, DataBufferPtr, &Flag, UserId);
                ...
                {
                    Adc_UpdateStatusReadGroup(Group, Flag, UserId);
                }
            }
#if (ADC_VALIDATE_PARAMS == STD_ON)
        }
...
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    UserId = Adc_GetUserID();
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_GETGROUPSTATUS_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */
        /* SWS_Adc_00220 */
...
void Adc_EnableHardwareTrigger(Adc_GroupType Group)
{
    ...
    UserId = Adc_GetUserID();
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_ENABLEHARDWARETRIGGER_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */
        /* Get the mapping index of group in the current partition */
        GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];

        if ((Std_ReturnType)E_OK == Adc_ValidateStateEnableHwTriggNotBusy(Group, UserId))
        {
#if (ADC_VALIDATE_PARAMS == STD_ON)
               ...
               )
            {
                ...
                /* Get the unit to which the group belongs to */
                LogicalHwUnitId = Adc_apxCfgPtr[UserId]->GroupsPtr[(GroupIndex)].AdcLogicalUnitId;
                Adc_UpdateStatusEnableHardware(Group, LogicalHwUnitId, UserId);

                ValidStatus = Adc_Ipw_EnableHardwareTrigger(Group, LogicalHwUnitId, UserId);
...
void Adc_DisableHardwareTrigger(Adc_GroupType Group)
{
    ...
    UserId = Adc_GetUserID();
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_DISABLEHARDWARETRIGGER_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */

        if ((Std_ReturnType)E_OK == Adc_ValidateStateNotIdle(ADC_DISABLEHARDWARETRIGGER_ID, Group, UserId))
        {
               ...
               )
            {
                ...
                /* Get the unit to which the group belongs to */
                LogicalHwUnitId = Adc_apxCfgPtr[UserId]->GroupsPtr[(GroupIndex)].AdcLogicalUnitId;
                Adc_UpdateStatusDisableHardware(Group, LogicalHwUnitId, UserId);

                Adc_Ipw_DisableHardwareTrigger(Group, LogicalHwUnitId, UserId);
...
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_ENABLEGROUPNOTIFICATION_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */

#if (ADC_VALIDATE_NOTIFY_CAPABILITY == STD_ON)
        if ((Std_ReturnType)E_OK == Adc_NotifyCapablity(ADC_ENABLEGROUPNOTIFICATION_ID, Group, UserId))
        {
#endif /* ADC_VALIDATE_NOTIFY_CAPABILITY == STD_ON */
            /* SWS_Adc_00057 */
...
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_DISABLEGROUPNOTIFICATION_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */

#if (ADC_VALIDATE_NOTIFY_CAPABILITY == STD_ON)
        if ((Std_ReturnType)E_OK == Adc_NotifyCapablity(ADC_DISABLEGROUPNOTIFICATION_ID, Group, UserId))
        {
#endif /* ADC_VALIDATE_NOTIFY_CAPABILITY == STD_ON */
            /* SWS_Adc_00058 */
            Adc_axGroupStatus[Group].Notification = ADC_NOTIFICATION_DISABLED;
...
{
    volatile uint8 UserId = Adc_GetUserID();
    Adc_GroupType GroupIndex = 0U;
...
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    Std_ReturnType ValidStatus = Adc_ValidateCallAndGroup(ADC_GETSTREAMLASTPOINTER_ID, Group, UserId);
...
#endif
#endif /* (ADC_ENABLE_GROUP_STREAMING_RESULTS_REORDER == STD_ON) */
        if ((Std_ReturnType)E_OK == Adc_ValidateStateNotIdle(ADC_GETSTREAMLASTPOINTER_ID, Group, UserId))
        {
            *PtrToSamplePtr = NULL_PTR;
                ...
                (ADC_BUSY != Adc_axGroupStatus[Group].Conversion)) /* SWS_Adc_00216 */
            {
                ...
                {
                    ResultIndex = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].NumSamples - (Adc_StreamNumSampleType)1U;
                    /* SWS_Adc_00387 */
                    NumberOfResults = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].NumSamples;
                }
                else
                {
...
#endif /* (ADC_ENABLE_GROUP_STREAMING_RESULTS_REORDER == STD_ON) */
                Adc_UpdateStatusAfterGetStream(Group, UserId);
            }
        }
        else
        {
...
Std_ReturnType Adc_SetPowerState(Adc_PowerStateRequestResultType * Result)
{
    ...
    UserId = Adc_GetUserID();
    UserIdTemp = UserId; /* Used to avoid MISRA */
    if ((Std_ReturnType)E_OK == Adc_ValidateStateAndPtr(ADC_SETPOWERSTATE_ID, Result, UserId))
    {
        if ((Std_ReturnType)E_OK == Adc_ValidatePowerStatePrepare(ADC_SETPOWERSTATE_ID, Result, UserId))
        {
            if ((Std_ReturnType)E_OK == Adc_ValidatePowerStateSupport(ADC_SETPOWERSTATE_ID, Adc_aeTargetState[UserId], Result))
            {
                RetVal = Adc_ValidateIdleState(ADC_SETPOWERSTATE_ID, Result, UserId);
            }
        }
    }
    ...
    {
        RetVal = Adc_Ipw_SetPowerState(Adc_aeTargetState[UserId], UserIdTemp);
        if ((Std_ReturnType)E_OK == RetVal)
        {
            /* SWS_Adc_00482 */
            /* Everything is ok */
            *Result = ADC_SERVICE_ACCEPTED;
            /* Use StateTemp and UserIdTemp to avoid IAR warning */
            StateTemp = Adc_aeTargetState[UserIdTemp];
            Adc_aeCurrentState[UserId] = StateTemp;
            Adc_aeTargetState[UserId] = ADC_NODEFINE_POWER;
        }
        else
                                        ...
                                        Adc_PowerStateRequestResultType * Result)
{
    ...
    UserId = Adc_GetUserID();
    if ((Std_ReturnType)E_OK == Adc_ValidateStateAndPtr(ADC_GETCURRENTPOWERSTATE_ID, Result, UserId))
    {
#if (ADC_VALIDATE_PARAMS == STD_ON)
        if ((Std_ReturnType)E_OK == Adc_ValidatePtr(ADC_GETCURRENTPOWERSTATE_ID, CurrentPowerState))
#endif /* (ADC_VALIDATE_PARAMS == STD_ON) */
        {
            *CurrentPowerState = Adc_aeCurrentState[UserId];
            *Result = ADC_SERVICE_ACCEPTED;
                                       ...
                                       Adc_PowerStateRequestResultType * Result)
{
    ...
    UserId = Adc_GetUserID();
    if ((Std_ReturnType)E_OK == Adc_ValidateStateAndPtr(ADC_GETTARGETPOWERSTATE_ID, Result, UserId))
    {
#if (ADC_VALIDATE_PARAMS == STD_ON)
...
#endif /* (ADC_VALIDATE_PARAMS == STD_ON) */
        {
            ...
            if (Adc_aeTargetState[UserId] < ADC_NODEFINE_POWER)
            {
                ...
                *TargetPowerState = Adc_aeTargetState[UserId];
            }
            else
            {
                /* No transition is ongoing */
                *TargetPowerState = Adc_aeCurrentState[UserId];
            }
            *Result = ADC_SERVICE_ACCEPTED;
            RetVal = (Std_ReturnType)E_OK;
        }
                                     ...
                                     Adc_PowerStateRequestResultType * Result)
{
    ...
    UserId = Adc_GetUserID();
    if ((Std_ReturnType)E_OK == Adc_ValidateStateAndPtr(ADC_PREPAREPOWERSTATE_ID, Result, UserId))
    {
        if ((Std_ReturnType)E_OK == Adc_ValidatePowerStateSupport(ADC_PREPAREPOWERSTATE_ID, PowerState, Result))
        {
            ...
            else
            {
                /* SWS_Adc_00495 */
                /* Everything is ok */
                Adc_aeTargetState[UserId] = PowerState;
                *Result = ADC_SERVICE_ACCEPTED;
            }
        }
    }
                                      ...
                                      Adc_SetPowerModeType SetPowerMode)
{
    ...
    UserId = Adc_GetUserID();
    if ((Std_ReturnType)E_OK == Adc_ValidateSetHwUnitPowerMode(SetPowerMode, UserId))
    {
        TempReturn = Adc_ValidateNotBusyNoQueue(Unit, ADC_SET_HW_UNIT_POWER_MODE_ID);

        if ((Std_ReturnType)E_OK == TempReturn)
        {
            if ((Std_ReturnType)E_OK == Adc_ValidateCheckGroupNotConversion(ADC_SET_HW_UNIT_POWER_MODE_ID, UserId))
            {
                TempReturn = Adc_Ipw_SetHwUnitPowerMode(Unit, SetPowerMode, UserId);
                if ((Std_ReturnType)E_NOT_OK == TempReturn)
                {
...
#if ((ADC_ENABLE_CTU_CONTROL_MODE_API == STD_ON) || (ADC_ENABLE_CTUTRIG_NONAUTO_API == STD_ON) \
     || (ADC_HW_TRIGGER_API == STD_ON))
                                   ...
                                   Adc_PowerStateType State)
{
    ...
    {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
        TempReturn = Adc_ValidateCheckGroupNotConversion(ADC_CTU_SET_POWER_MODE_ID, UserId);
        if ((Std_ReturnType)E_OK == TempReturn)
        {
...
void Adc_EnableCTUTrigger(Adc_GroupType Group,
                          Adc_HwTriggerTimerType TriggerSource)
{
    const volatile uint8 UserId = Adc_GetUserID();
    ...
    ValidStatus = Adc_ValidateCallAndGroup((uint8)ADC_ENABLECTUTRIGGER_ID, Group, UserId);
    if (ValidStatus == (Std_ReturnType)E_OK)
    {
#endif /* (ADC_VALIDATE_CALL_AND_GROUP  == STD_ON) */
        /* Map TriggerSource to TriggerIndex */
        TriggFound = Adc_Ipw_MapCTUTriggerIndex(TriggerSource, UserId, &TrigIdx);
        (void)TriggFound;
#if (ADC_VALIDATE_CALL_AND_GROUP  == STD_ON)
...
#endif /* (ADC_VALIDATE_CALL_AND_GROUP  == STD_ON) */
            GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
            GroupConfig = &(Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex]);
            LogicalHwUnitId = GroupConfig->AdcLogicalUnitId;

            if ((Std_ReturnType)E_OK == Adc_ValidateNotBusyEnableHwTrig(ADC_ENABLECTUTRIGGER_ID, Group, UserId))
            {
#if (ADC_VALIDATE_PARAMS == STD_ON)
    ...
    if (TRUE == IsValidateOk)
    {
        ...
        if (0U == Adc_au8CtuGroupTriggersActive[LogicalHwUnitId][TrigIdx])
        {
            ...
            ValidStatus = Adc_Ipw_EnableCtuTrigger(Group, TriggerSource, FirstTrigger, UserId);
            if ((Std_ReturnType)E_NOT_OK == ValidStatus)
            {
                /* There is a running conversion.*/
                           ...
                           Adc_HwTriggerTimerType TriggerSource)
{
...
#if (ADC_VALIDATE_CALL_AND_GROUP  == STD_ON)
    boolean IsValidateOk = FALSE;
    Std_ReturnType ValidStatus = Adc_ValidateCallAndGroup((uint8)ADC_DISABLECTUTRIGGER_ID, Group, UserId);
    if (ValidStatus == (Std_ReturnType)E_OK)
    {
#endif /* (ADC_VALIDATE_CALL_AND_GROUP  == STD_ON) */
        /* Map TriggerSource to TriggerIndex */
    ...
    if (TRUE == IsValidateOk)
    {
        ...
        if (0U != Adc_au8CtuGroupTriggersActive[LogicalHwUnitId][TrigIdx])
        {
            ...
            Adc_Ipw_DisableCtuTrigger(Group, TriggerSource, LastTrigger, UserId);
#if (ADC_VALIDATE_PARAMS == STD_ON)
        }
        else
        {
...
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_HWRESULTREADGROUP_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */
...
#endif
        {
            GroupRet = Adc_Ipw_HwResultReadGroup(Group, DataPtr, UserId);
        }
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    }
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */
                     ...
                     Adc_ValueGroupType * const DataBufferPtr)
{
    ...
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_ENABLE_READ_RAW_DATA_ID, Unit, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */
#if (ADC_VALIDATE_PARAMS == STD_ON)
            ...
            if ((Std_ReturnType)E_OK == Adc_ValidatePtr(ADC_ENABLE_READ_RAW_DATA_ID, ChansArray))
            {
                ...
                if (NumItems <= ADC_IPW_RES_REG_NUM)
                {
                    ...
                    for (i = 0U; i < NumItems; i++)
                    {
                        ...
                        {
                            Status = Adc_Ipw_GetConvData(Unit, ChansArray[i], UserId, &DataBufferPtr[i]);
                        }
#if (ADC_VALIDATE_PARAMS == STD_ON)
                        else
                        {
                       ...
                       Adc_ChannelType Channel)
{
    ...
    UserId = Adc_GetUserID();
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_ENABLE_CHANNEL_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */
    ...
    LogicalChannelId = Channel & ADC_CHANNEL_SYMBOLIC_NAME_MASK_CHANNEL_ID_U16;
        ...
        {
            if ((Std_ReturnType)E_OK == Adc_ValidateEnableDisbleIdList(ADC_ENABLE_CHANNEL_ID, Group, LogicalChannelId, UserId))
            {
#endif /* ADC_VALIDATE_PARAMS == STD_ON */
                /* Get the unit to which the group belongs to */
                GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
                LogicalHwUnitId = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AdcLogicalUnitId;
                PhysicalChannelId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcConfigPtrArr[LogicalHwUnitId]->ChannelConfigsPtr[LogicalChannelId].ChanIndex;
                GroupId = (Adc_GroupType)Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].EnableChDisableChGroupIndex;

                Adc_Ipw_GetCmrRegister(LogicalHwUnitId, Group, &CmrMask, UserId);
                        ...
                        Adc_ChannelType Channel)
{
    ...
    UserId = Adc_GetUserID();
#if (ADC_VALIDATE_CALL_AND_GROUP == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndGroup(ADC_DISABLE_CHANNEL_ID, Group, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_GROUP == STD_ON */
    ...
    LogicalChannelId = Channel & ADC_CHANNEL_SYMBOLIC_NAME_MASK_CHANNEL_ID_U16;
        ...
        {
            if ((Std_ReturnType)E_OK == Adc_ValidateEnableDisbleIdList(ADC_DISABLE_CHANNEL_ID, Group, LogicalChannelId, UserId))
            {
#endif /* ADC_VALIDATE_PARAMS == STD_ON */
                GroupIndex = Adc_apxCfgPtr[UserId]->GroupIdToIndexMapPtr[Group];
                LogicalHwUnitId = Adc_apxCfgPtr[UserId]->GroupsPtr[GroupIndex].AdcLogicalUnitId;
...
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    UserId = Adc_GetUserID();
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_GETINJECTEDCONVERSIONSTATUS_ID, Unit, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */
#if (ADC_SOFTWARE_INJECTED_CONVERSIONS_USED == STD_ON)
                   ...
                   Adc_CalibrationStatusType * pStatus)
{
    ...
    UserId = Adc_GetUserID();
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_CALIBRATE_ID, Unit, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */
        ...
        if ((Std_ReturnType)E_OK == Adc_ValidatePtr(ADC_CALIBRATE_ID, pStatus))
        {
            ...
            {
                if ((Std_ReturnType)E_OK == Adc_ValidateCheckGroupNotConversion(ADC_CALIBRATE_ID, UserId))
                {
                    Adc_Ipw_Calibrate(Unit, pStatus, UserId);
...
#if (ADC_USER_GAIN_OFFSET == STD_ON)
/** @implements      Adc_SetUserGainAndOffset_Activity */
Std_ReturnType Adc_SetUserGainAndOffset(Adc_HwUnitType Unit,
                                        const Adc_UserGainType Gain,
                                        const Adc_UserOffsetType Offset)
{
    Std_ReturnType ReturnValue = (Std_ReturnType)E_OK;
    const uint8 UserId = Adc_GetUserID();

#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    ReturnValue = Adc_ValidateCallAndUnit(ADC_SET_USER_GAIN_AND_OFFSET_ID, Unit, UserId);
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */

    if ((Std_ReturnType)E_OK == ReturnValue)
    {
        ReturnValue = Adc_Ipw_SetUserGainAndOffset(Unit, UserId, Gain, Offset);
    }

    return ReturnValue;
}
#endif /* ADC_USER_GAIN_OFFSET == STD_ON */
#if (ADC_APPLY_CALIBRATION == STD_ON)
/** @implements      Adc_ApplyCalibration_Activity */
Std_ReturnType Adc_ApplyCalibration(Adc_HwUnitType Unit,
...
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    ReturnValue = Adc_ValidateCallAndUnit(ADC_APPLY_CALIBRATION_ID, Unit, UserId);
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */

#if (ADC_VALIDATE_PARAMS == STD_ON)
    if ((Std_ReturnType)E_OK == ReturnValue)
    {
        ReturnValue = Adc_ValidatePtr(ADC_APPLY_CALIBRATION_ID, UncalibratedBuffer);
    }

    if ((Std_ReturnType)E_OK == ReturnValue)
    {
        ReturnValue = Adc_ValidatePtr(ADC_APPLY_CALIBRATION_ID, CalibratedBuffer);
    }
#endif /* ADC_VALIDATE_PARAMS == STD_ON */

    if ((Std_ReturnType)E_OK == ReturnValue)
    {
        ReturnValue = Adc_Ipw_ApplyCalibration(Unit, UserId, BufferLength, UncalibratedBuffer, CalibratedBuffer);
    }
...
Std_ReturnType Adc_SelfTest(Adc_HwUnitType Unit)
{
    ...
    UserId = Adc_GetUserID();
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_SELFTEST_ID , Unit, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */

        ReturnValue = Adc_Ipw_SelfTest(Unit, UserId);
                            ...
                            Adc_WdgThresholdValueType HighValue)
{
    ...
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_CONFIGURE_THRESHOLD_ID, LogicalHwUnitId, UserId))
    {
#endif /* if (ADC_DEV_ERROR_DETECT == STD_ON) */

        Status = Adc_Ipw_ConfigureThreshold(ThresholdControlIndex, LowValue, HighValue, UserId);
...
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_ENABLEWDGNOTIFICATION_ID, LogicalHwUnitId, UserId))
    {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */

        Status = Adc_Ipw_EnableWdgNotification(LogicalHwUnitId, ChannelId, UserId);
...
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_DISABLEWDGNOTIFICATION_ID, LogicalHwUnitId, UserId))
    {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */

        Status = Adc_Ipw_DisableWdgNotification(LogicalHwUnitId, ChannelId, UserId);
...
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    UserIdTemp = UserId; /* Used to avoid MISRA */
    if (TRUE == Adc_CheckSetClockModeCoreAndParam(Prescaler, UserIdTemp))
    {
#endif /* ADC_DEV_ERROR_DETECT == STD_ON */
        for (LogicalHwUnitId = 0U; LogicalHwUnitId < ADC_IPW_MAX_HW_UNITS; LogicalHwUnitId++)
        {
            /* HW unit enabled on current partition */
            if (Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.AdcHwUnitArr[LogicalHwUnitId] == (uint8)STD_ON)
            {
        ...
        if ((Std_ReturnType)E_OK == ErrorFound)
        {
            if ((Std_ReturnType)E_OK == Adc_ValidateCheckGroupNotConversion(ADC_SETCLOCKMODE_ID, UserId))
            {
                Ret = Adc_Ipw_SetClockMode(Prescaler, UserId);
#if (ADC_SET_ADC_CONV_TIME_ONCE == STD_OFF)
                Adc_aeClockMode[UserId] = Prescaler;
#endif
            }
        }
#if (ADC_DEV_ERROR_DETECT == STD_ON)
                    ...
                    Adc_ChannelIndexType NumberOfChannel)
{
...
#endif /* ((ADC_DEV_ERROR_DETECT == STD_ON) ....) */
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    uint8 UserIdTemp = UserId; /* Used to avoid MISRA */
    uint32 Index = 0u;
    Std_ReturnType StatusChecks = Adc_ValidateCallAndGroup(ADC_SETCHANNEL_ID, Group, UserId);
    ...
    if (StatusChecks == (Std_ReturnType)E_OK)
    {
        ...
        if ((Std_ReturnType)E_OK == Adc_ValidatePtr(ADC_SETCHANNEL_ID, Channel))
        {
            ...
            for (Index = 0; Index < NumberOfChannel; Index++)
            {
                ...
                if (
                    (Channel[Index] >= Adc_apxCfgPtr[UserIdTemp]->AdcIpwConfigPtr->Mapping.AdcChannels[LogicalHwUnitId])
#if (ADC_ENABLE_LIMIT_CHECK == STD_ON)
                    || ((NumberOfChannel > 1U) &&
#if ((ADC_IPW_BCTU_AVAILABLE == STD_ON) || (ADC_IPW_CTU_AVAILABLE == STD_ON))
                        (TRUE == Adc_apxCfgPtr[UserIdTemp]->AdcIpwConfigPtr->AdcIpwChannelConfig[LogicalHwUnitId][Channel[Index]].ChannelLimitCheckingConfig.ChannelLimitCheckEnabled)
#else
                        (TRUE == Adc_apxCfgPtr[UserIdTemp]->AdcIpwConfigPtr->ChannelLimitCheckingConfigs[LogicalHwUnitId][Channel[Index]].ChannelLimitCheckEnabled)
    ...
    {
#if (ADC_CTU_HW_TRIGGER_OPTIMIZATION == STD_OFF)
        StatusChecks = Adc_CheckSetChannelCtuTriggers(GroupIndex, Channel, LogicalHwUnitId, NumberOfChannel, UserId);
#else
        StatusChecks = Adc_CheckSetChannelCtuTriggers(GroupIndex, UserId);
#endif /* (ADC_CTU_HW_TRIGGER_OPTIMIZATION == STD_OFF) */
    }
#endif /* ((ADC_HW_TRIGGER_API == STD_ON) && (ADC_IPW_CTU_TRIGGER_MODE_SUPPORTED == STD_ON)) */
    if (StatusChecks == (Std_ReturnType)E_OK)
...
#if ((ADC_IPW_BCTU_AVAILABLE == STD_ON) || (ADC_IPW_CTU_AVAILABLE == STD_ON))
        Adc_axRuntimeGroupChannel[Group].AdcRuntimeGroupLimitcheck = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcIpwChannelConfig[LogicalHwUnitId][Channel[0U]].ChannelLimitCheckingConfig.ChannelLimitCheckEnabled;
#else
        Adc_axRuntimeGroupChannel[Group].AdcRuntimeGroupLimitcheck = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->ChannelLimitCheckingConfigs[LogicalHwUnitId][Channel[0U]].ChannelLimitCheckEnabled;
#endif /* ((ADC_IPW_BCTU_AVAILABLE == STD_ON) || (ADC_IPW_CTU_AVAILABLE == STD_ON)) */
#endif /* (ADC_ENABLE_LIMIT_CHECK == STD_ON) */
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    }
...
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_ENABLE_CTU_CONTROL_MODE_ID, Unit, UserId))
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    {
        if ((Std_ReturnType)E_OK == Adc_ValidateUnitActive(ADC_ENABLE_CTU_CONTROL_MODE_ID, Unit, UserId))
        {
#endif /* ADC_DEV_ERROR_DETECT == STD_ON */
            if ((Std_ReturnType)E_OK == Adc_ValidateStateCtuControlMode(ADC_ENABLE_CTU_CONTROL_MODE_ID, Unit))
            {
                if ((Std_ReturnType)E_NOT_OK == Adc_Ipw_EnableCtuControlMode(Unit, UserId))
                {
#if (ADC_DEV_ERROR_DETECT == STD_ON)
...
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_DISABLE_CTU_CONTROL_MODE_ID, Unit, UserId))
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    {
        if ((Std_ReturnType)E_OK == Adc_ValidateUnitActive(ADC_DISABLE_CTU_CONTROL_MODE_ID, Unit, UserId))
        {
#endif /* ADC_DEV_ERROR_DETECT == STD_ON */
            if ((Std_ReturnType)E_OK == Adc_ValidateStateCtuControlMode(ADC_DISABLE_CTU_CONTROL_MODE_ID, Unit))
            {
                Adc_Ipw_DisableCtuControlMode(Unit, UserId);
            }
#if (ADC_DEV_ERROR_DETECT == STD_ON)
        }
    }
...
void Adc_CtuEnableHwTrigger(Adc_CtuTrigSrcType TrigSource)
{
    ...
    uint8 UserId = Adc_GetUserID();

#if (ADC_DEV_ERROR_DETECT == STD_ON)
    const uint8 ServiceId = ADC_CTU_ENABLE_HW_TRIGGER_ID;

    if (Adc_ValidateCtuControlModeApi(CtuLogicalId, ServiceId, UserId) == (Std_ReturnType)E_OK)
    {
        ...
        else
        {
            ...
            Adc_Ipw_CtuEnableHwTrigger(CtuLogicalId, (uint8)TriggerId, UserId);
...
void Adc_CtuDisableHwTrigger(Adc_CtuTrigSrcType TrigSource)
{
    ...
    if (Adc_ValidateCtuControlModeApi(CtuLogicalId, ServiceId, UserId) == (Std_ReturnType)E_OK)
    {
        ...
        else
        {
            ...
            Adc_Ipw_CtuDisableHwTrigger(CtuLogicalId, (uint8)TriggerId, UserId);
...
void Adc_CtuStartConversion(Adc_CtuTrigSrcType TrigSource)
{
    ...
    if (Adc_ValidateCtuControlModeApi(CtuLogicalId, ServiceId, UserId) == (Std_ReturnType)E_OK)
    {
        ...
        else
        {
            ...
            Adc_Ipw_CtuStartConversion(CtuLogicalId, (uint8)TriggerId, UserId);
...
Adc_ValueGroupType Adc_CtuReadConvData(Adc_HwUnitType AdcUnit)
{
    ...
    if (Adc_ValidateCtuControlModeApiByAdcUnit(AdcUnit, ServiceId, UserId) == (Std_ReturnType)E_OK)
    {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
        AdcPhysicalId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcPhysicalIdArr[AdcUnit];
        ConvData = Adc_Ipw_CtuReadConvData(AdcPhysicalId);
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    }
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
...
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
        AdcPhysicalId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->AdcPhysicalIdArr[AdcUnit];
        Adc_Ipw_CtuReadConvResult(AdcPhysicalId, Result);
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    }
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
}
...
void Adc_CtuStopLoopConversions(Adc_CtuTrigSrcType TrigSource)
{
    ...
    if (Adc_ValidateCtuControlModeApi(CtuLogicalId, ServiceId, UserId) == (Std_ReturnType)E_OK)
    {
        ...
        CtuPhysicalId = Adc_apxCfgPtr[UserId]->AdcIpwConfigPtr->Mapping.CtuLogToPhyId[CtuLogicalId];
        ...
        else if (FALSE == Adc_ValidateTriggerLoopMode(UserId, CtuPhysicalId, TriggerId))
        {
            Adc_ReportDetError(ServiceId, ADC_E_INVALID_PARAM);
        }
        else
        {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
                         ...
                         uint8 DataLength)
{
    ...
    if ((Std_ReturnType)E_OK == Adc_ValidateCtuControlModeApi(CtuLogicalId, ServiceId, UserId))
    {
        /* CPR_RTD_00471.adc */
        if ((Std_ReturnType)E_OK == Adc_ValidateCtuFifoParam(ServiceId, UserId, CtuLogicalId, FifoIndex, DataLength))
        {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
            for (Index = 0u; Index < DataLength; Index++)
            {
                Data[Index] = Adc_Ipw_CtuReadFifoData(CtuLogicalId, FifoIndex, UserId);
            }
#if (ADC_DEV_ERROR_DETECT == STD_ON)
        }
    }
                           ...
                           uint8 ResultLength)
{
    ...
    {
        /* CPR_RTD_00468.adc */
        if ((Std_ReturnType)E_OK == Adc_ValidateCtuFifoParam(ServiceId, UserId, CtuLogicalId, FifoIndex, ResultLength))
        {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
            for (Index = 0u; Index < ResultLength; Index++)
            {
                Adc_Ipw_CtuReadFifoResult(CtuLogicalId, FifoIndex, &(Result[Index]), UserId);
            }
#if (ADC_DEV_ERROR_DETECT == STD_ON)
        }
    }
                             ...
                             uint8 Watermark)
{
    ...
    if (Adc_ValidateCtuControlModeApi(CtuLogicalId, ServiceId, UserId) == (Std_ReturnType)E_OK)
    {
        ...
        {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
            Adc_Ipw_CtuSetFifoWatermark(CtuLogicalId, FifoIndex, Watermark, UserId);
#if (ADC_DEV_ERROR_DETECT == STD_ON)
        }
    }
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
...
void Adc_CtuEnableNotification(Adc_CtuNotificationType Notification)
{
    ...
    {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
            Adc_Ipw_CtuEnableNotification(UserId, CtuLogicalId, Notification);
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    }
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
}
...
void Adc_CtuDisableNotification(Adc_CtuNotificationType Notification)
{
    ...
    {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
            Adc_Ipw_CtuDisableNotification(UserId, CtuLogicalId, Notification);
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    }
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
}
                    ...
                    const uint8 ListStartPosition)
{
    ...
    if (Adc_ValidateCtuControlModeApi(CtuUnit, ServiceId, UserId) == (Std_ReturnType)E_OK)
    {
        ...
        {
            if ((NumItems >= ADC_IPW_NUM_CTU_LIST_ELEMS(CtuUnit)) || (ListStartPosition >= ADC_IPW_NUM_CTU_LIST_ELEMS(CtuUnit)))
            {
                Adc_ReportDetError(ServiceId, ADC_E_WRONG_TRIGG_SRC);
            }
            else
            {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
                Adc_Ipw_CtuSetList(CtuUnit, ListStartPosition, ListItemsArray, NumItems, UserId);
#if (ADC_DEV_ERROR_DETECT == STD_ON)
            }
        }
    }
                           ...
                           Adc_CtuListPtrType ListPtr)
{
    ...
    {
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
        Adc_Ipw_CtuSetListPointer(CtuLogicalId, TriggerId, ListPtr, UserId);
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    }
#endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
}
...
uint16 Adc_TempSenseCalculateTemp(Adc_HwUnitType Unit,
                                  const uint16 TempSenseAdcConvData)
{
    uint16 TempSenseVal = 0u;
#if (ADC_DEV_ERROR_DETECT == STD_ON)
    const volatile uint8 UserId = Adc_GetUserID();

    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_TEMPSENSE_CALCULATE_TEMP_ID, Unit, UserId))
    {
        if (TempSenseAdcConvData > ADC_SAR_IP_TEMPSENSE_CONVDATA_MAX)
        {
            /* The data conversion must not exceeed the maximum conversion result */
...
Std_ReturnType Adc_TempSenseGetTemp(Adc_HwUnitType Unit,
                                    uint16 * const TempSenseVal)
{
    ...
    const volatile uint8 UserId = Adc_GetUserID();
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
    if ((Std_ReturnType)E_OK == Adc_ValidateCallAndUnit(ADC_TEMPSENSE_GET_TEMP_ID, Unit, UserId))
    {
#endif /* ADC_VALIDATE_CALL_AND_UNIT == STD_ON */
        if ((Std_ReturnType)E_OK == Adc_ValidateNotBusyNoQueue(Unit, ADC_TEMPSENSE_GET_TEMP_ID))
        {
            if ((Std_ReturnType)E_OK == Adc_ValidateCheckGroupNotConversion(ADC_TEMPSENSE_GET_TEMP_ID, UserId))
            {
                RetVal = Adc_Ipw_TempSenseGetTemp(Unit, 0u, TempSenseVal, UserId);
...
Std_ReturnType Adc_TempSenseSetPowerMode(Adc_SetPowerModeType SetPowerMode)
{
