...
/**
*   @file    Can_43_FLEXCAN_Irq.c
*   @version 7.0.1
*
*   @brief   AUTOSAR Can_43_FLEXCAN - LLD module implementation.
*   @details High Level Driver interrupt callback implementation.
*
...
#define CAN_43_FLEXCAN_IRQ_AR_RELEASE_MINOR_VERSION_C     9
#define CAN_43_FLEXCAN_IRQ_AR_RELEASE_REVISION_VERSION_C  0
#define CAN_43_FLEXCAN_IRQ_SW_MAJOR_VERSION_C             7
#define CAN_43_FLEXCAN_IRQ_SW_MINOR_VERSION_C             0
#define CAN_43_FLEXCAN_IRQ_SW_PATCH_VERSION_C             1
...
/* Function process Message Buffer Event */
static void Can_43_FLEXCAN_ProcessMBInterruptEvent
(
    uint8 u8Instance,
...
#if (CAN_43_FLEXCAN_MB_INTERRUPT_SUPPORT == STD_ON)
/**
*   @brief      Function process the event in MB Interrupt.
*/
static void Can_43_FLEXCAN_ProcessMBInterruptEvent
(
    uint8 u8Instance,
...
{
    if (u32BuffIdx < (uint32)CAN_43_FLEXCAN_HWMB_COUNT)
    {
        switch (Event)
        {
            case FLEXCAN_EVENT_TX_COMPLETE:
            /* case FLEXCAN_EVENT_TX_ABORT: not supported on HLD yet */
            {
                Can_43_FLEXCAN_ProcessMesgBufferCommonInterrupt(u8Instance, (uint8)u32BuffIdx, CAN_43_FLEXCAN_TX_NORMAL);
                break;
            }
            case FLEXCAN_EVENT_RX_COMPLETE:
            {
                Can_43_FLEXCAN_ProcessMesgBufferCommonInterrupt(u8Instance, (uint8)u32BuffIdx, CAN_43_FLEXCAN_RX_NORMAL);
                break;
            }
            case FLEXCAN_EVENT_RXFIFO_COMPLETE:
            {
                Can_43_FLEXCAN_ProcessMesgBufferCommonInterrupt(u8Instance, (uint8)u32BuffIdx, CAN_43_FLEXCAN_RX_LEGACY_FIFO);
                break;
            }
            case FLEXCAN_EVENT_RXFIFO_WARNING:
            {
                Can_43_FLEXCAN_ProcessMesgBufferCommonInterrupt(u8Instance, 6U, CAN_43_FLEXCAN_RX_LEGACY_FIFO);
                break;
            }
            case FLEXCAN_EVENT_RXFIFO_OVERFLOW:
            {
                Can_43_FLEXCAN_ProcessMesgBufferCommonInterrupt(u8Instance, 7U, CAN_43_FLEXCAN_RX_LEGACY_FIFO);
                break;
            }
            default:
            {
                          ...
                          )
{
    ...
    if (CAN_43_FLEXCAN_MB_ENHANCED_RXFIFO == u32buffIdx)
    {
        if (FALSE == driverState->enhancedFifoOutput.isPolling)
        {
            switch (event)
            {
                ...
                {
                    Can_43_FLEXCAN_ProcessMesgBufferCommonInterrupt(u8Instance, (uint8)event, CAN_43_FLEXCAN_RX_ENHANCED_FIFO);
                    break;
                }
                default:
                {
...
#endif /* CAN_43_FLEXCAN_FEATURE_HAS_ENHANCED_RX_FIFO */
    {
        ...
        {
            Can_43_FLEXCAN_ProcessMBInterruptEvent(u8Instance, event, u32buffIdx);
        ...
        {
            Can_43_FLEXCAN_ProcessMesgBufferCommonInterrupt(u8Instance, (uint8)event, CAN_43_FLEXCAN_RX_LEGACY_FIFO);
        }
        else
        {
            /* prevent misra */
